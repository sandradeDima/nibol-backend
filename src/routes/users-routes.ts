...
const getRequiredUserId = (value: string | string[] | undefined): string => {
...
usersRouter.patch(
  ...
  requirePermission("users.edit"),
  asyncHandler(async (request, response) => {
    const user = await usersService.setUserActiveState(
      getRequiredUserId(request.params.userId),
      false,
      getRequestLogActorContext(request),
    );
...
usersRouter.post(
  ...
  asyncHandler(async (request, response) => {
    ...
    const deletedCount = await usersService.bulkDeleteUsers(
      payload.ids,
      getRequestLogActorContext(request),
    );
