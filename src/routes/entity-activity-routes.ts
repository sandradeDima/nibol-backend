...
const getAccess = (request: Request) => {
  if (!request.authorizationSummary) throw new AppError("Authorization required.", 401);
...
entityActivityRouter.get(
  ...
  asyncHandler(async (request, response) => {
    const query = parseQuery(request);
    const result = await entityActivityService.list(
      { ...query, entityId: getParamValue(request.params.entityId), entityType: getParamValue(request.params.entityType) },
      getAccess(request),
    );
    sendPaginated(response, result.data, result.pagination);
  }),
