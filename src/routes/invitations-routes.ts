...
invitationsRouter.get(
  ...
  requirePermission("invitations.view"),
  asyncHandler(async (request, response) => {
    const invitation = await invitationsService.getInvitationById(
      getRequiredInvitationId(request.params.id),
    );
...
invitationsRouter.post(
  ...
  requirePermission("invitations.edit"),
  asyncHandler(async (request, response) => {
...
invitationsRouter.post(
  ...
  asyncHandler(async (request, response) => {
    const invitation = await invitationsService.revokeInvitation(
      getRequiredInvitationId(request.params.id),
      getRequestLogActorContext(request),
    );
...
invitationsRouter.post(
  ...
  asyncHandler(async (request, response) => {
    ...
    if (payload.mode === "preview") {
      const preview = await invitationsService.getInvitationAcceptancePreview(
        payload.token,
      );
    ...
    const acceptedInvitation = await invitationsService.acceptInvitation(
      payload,
      getRequestLogActorContext(request),
    );
