...
const event = (overrides: Partial<WorkflowNotificationEvent> = {}) =>
  ({
    ...
    visitSequence: 2,
    ...overrides,
  }) satisfies WorkflowNotificationEvent;
...
test("las claves de notificación son estables y separan canal y visita", () => {
  ...
  assert.equal(
    base,
    buildWorkflowNotificationDedupeKey(event(), "user-1", "EMAIL"),
  );
  assert.notEqual(
    base,
    buildWorkflowNotificationDedupeKey(event(), "user-1", "IN_APP"),
  );
  assert.notEqual(
    base,
    buildWorkflowNotificationDedupeKey(
      event({ visitSequence: 3 }),
      "user-1",
      "EMAIL",
    ),
  );
