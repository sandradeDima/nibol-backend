...
export type RuntimeConditionEvaluation = {
  groupId: string | null;
  evaluation: ConditionGroupEvaluation;
};

export type RuntimeTransitionSelection = {
  conditionEvaluations: RuntimeConditionEvaluation[];
...
const selectDefaultTransition = (
  graph: PinnedWorkflowGraph,
  node: PinnedWorkflowGraph["nodes"][number],
): RuntimeTransitionSelection => {
  ...
  return selectionError(
    ...
    `El nodo ${node.name} no tiene una ruta automática determinista.`,
  );
};

const selectConditionTransition = (
  graph: PinnedWorkflowGraph,
  node: PinnedWorkflowGraph["nodes"][number],
  ...
  const evaluations: RuntimeConditionEvaluation[] = [];
  ...
  return {
    ...selectionError(
      WORKFLOW_RUNTIME_ERROR_CODES.CONDITION_NO_ROUTE,
...
const selectHumanTransition = (
  ...
  if (
    ...
    !configuration.allowedActions.includes(normalizedDecision as never)
  ) {
    return selectionError(
      WORKFLOW_RUNTIME_ERROR_CODES.ACTION_NOT_ALLOWED,
      `La acción ${normalizedDecision} no está permitida en el nodo ${node.name}.`,
    );
  }
  ...
  const exact = outgoing.find(
    (transition) =>
      normalizeTransitionType(transition.transitionType) === normalizedDecision,
  );
  if (exact) {
    return {
      conditionEvaluations: [],
  ...
  return selectionError(
    WORKFLOW_RUNTIME_ERROR_CODES.TRANSITION_NOT_FOUND,
    `No existe una ruta controlada para la acción ${normalizedDecision}.`,
  );
};
...
const selectRejectionTransition = (
  ...
  const preferredTypes =
    configuration.behavior === "REQUEST_CORRECTION"
      ? ["CORRECTION", "RETURN"]
      : configuration.behavior === "RETURN_TO_STAGE"
        ? ["RETURN", "CORRECTION"]
        : ["REJECT"];
  const preferred = outgoing.find((transition) =>
    preferredTypes.includes(normalizeTransitionType(transition.transitionType)),
  );
  if (preferred) {
    return {
      conditionEvaluations: [],
...
export const selectRuntimeTransition = ({
  context,
  decision,
  graph,
  node,
  ...
  switch (node.configurationJson.nodeType) {
    ...
    case "APPROVAL":
      return decision
        ...
        : selectionError(
            WORKFLOW_RUNTIME_ERROR_CODES.TRANSITION_NOT_FOUND,
            `El nodo ${node.name} requiere una decisión humana.`,
          );
    case "REJECTION":
...
export const summarizeConditionEvaluations = (
  evaluations: RuntimeConditionEvaluation[],
) =>
  evaluations.map(({ evaluation, groupId }) => ({
    groupId,
