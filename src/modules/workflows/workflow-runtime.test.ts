...
const node = (input: Record<string, unknown>) => input;
...
} as PinnedWorkflowGraph;

const context = (riskLevel?: string) =>
  buildWorkflowRuntimeContext({
    actorUserId: "user-1",
...
test("runtime selects condition routes in priority order and uses fallback", () => {
  const condition = pinned.nodes.find(
    (candidate) => candidate.nodeKey === "condition",
  );
  assert.ok(condition);
...
test("runtime requires an explicit human decision and maps the controlled route", () => {
  const approval = pinned.nodes.find(
    (candidate) => candidate.nodeKey === "approval",
  );
  assert.ok(approval);
