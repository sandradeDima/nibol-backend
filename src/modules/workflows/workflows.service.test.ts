...
import {
  assertCanPublishWorkflowVersion,
  assertDraftWorkflowVersion,
  assertUniqueWorkflowNodeKeys,
  buildDefinitionOrderBy,
  buildDefinitionWhere,
  assertWorkflowCanStartInstance,
...
test("una versión publicada no puede modificarse", () => {
  assert.throws(
    () => assertDraftWorkflowVersion("PUBLISHED"),
...
test("un workflow archivado no puede iniciar una instancia", () => {
  assert.throws(
    () => assertWorkflowCanStartInstance("ARCHIVED"),
    (error: unknown) => error instanceof AppError && error.statusCode === 409,
  );
...
test("usuarios no autorizados no pueden publicar workflows", () => {
  assert.throws(
    () => assertCanPublishWorkflowVersion(unauthorizedAccess),
    (error: unknown) => error instanceof AppError && error.statusCode === 403,
  );
});
...
test("los nodeKey son únicos dentro de una versión", () => {
  ...
  assert.throws(
    () => assertUniqueWorkflowNodeKeys(["start", "review", "start"]),
    (error: unknown) => error instanceof AppError && error.statusCode === 409,
  );
});
...
test("la validación marca una asignación de etapa incompleta", () => {
  ...
  assert.ok(
    validation.errors.some((issue) => issue.code === "ASSIGNMENT_INVALID"),
  );
});
...
test("los permisos workflow se pueden sembrar de forma idempotente", () => {
  ...
  assert.equal(
    new Set(WORKFLOW_PERMISSION_NAMES).size,
