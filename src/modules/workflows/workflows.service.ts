...
import {
  ...
  WORKFLOW_DEFINITION_ENTITY_TYPE,
  WORKFLOW_PROCESS_TYPE_CATALOG,
  WORKFLOW_TRANSITION_TYPE_VALUES,
  WORKFLOW_VERSION_ENTITY_TYPE,
...
import {
  canonicalizeWorkflowConditionField,
...
import {
  WORKFLOW_INSTANCE_PERMISSIONS,
  WORKFLOW_PERMISSIONS,
} from "./workflows.permissions.js";
import type {
  ...
  WorkflowDesignerValidateInput,
  WorkflowNodeSaveInput,
  WorkflowTransitionSaveInput,
  ListWorkflowsQuery,
...
type WorkflowDatabase = Prisma.TransactionClient;

const workflowTransitionsOrderBy: Prisma.WorkflowTransitionOrderByWithRelationInput[] =
  [{ priority: "asc" }, { id: "asc" }];
...
type WorkflowDefinitionSelected = Prisma.WorkflowDefinitionGetPayload<{
  select: typeof workflowDefinitionSelect;
}>;

const mapWorkflowDefinition = (record: WorkflowDefinitionSelected) => {
...
type WorkflowVersionDetailSelected = Prisma.WorkflowVersionGetPayload<{
  select: typeof workflowVersionDetailSelect;
}>;

const mapWorkflowVersionDetail = (version: WorkflowVersionDetailSelected) => ({
  ...version,
...
const toInputJsonValue = (value: Prisma.JsonValue): Prisma.InputJsonValue => {
...
const assertAnyPermission = (
  ...
  if (
    ...
    throw new AppError(
      `Falta uno de los permisos requeridos: ${permissions.join(", ")}.`,
      403,
    );
  }
};

export const assertDraftWorkflowVersion = (status: string): void => {
  if (status !== "DRAFT") {
    throw new AppError(
      "Las versiones publicadas o archivadas no se pueden modificar.",
      409,
    );
  }
};

export const assertWorkflowCanStartInstance = (status: string): void => {
  if (status !== "PUBLISHED") {
    throw new AppError(
      "Solo se pueden iniciar instancias con workflows publicados.",
      409,
    );
  }
};

export const assertCanPublishWorkflowVersion = (
  access: AuthorizationSummary,
): void => {
  assertPermission(access, WORKFLOW_PERMISSIONS.publish);
};

export const assertUniqueWorkflowNodeKeys = (nodeKeys: string[]): void => {
  const uniqueKeys = new Set(nodeKeys);

  if (uniqueKeys.size !== nodeKeys.length) {
    throw new AppError(
      "Los nodeKey deben ser únicos dentro de la versión.",
      409,
    );
  }
};
...
const addDesignerIssue = (
  issues: WorkflowDesignerValidationResult,
  issue: WorkflowDesignerValidationResult["errors"][number],
): void => {
...
const assertAssignmentConfiguration = (
  node: WorkflowNodeSaveInput,
): string | null => {
  if (
    node.configurationJson.nodeType !== "STAGE" &&
  ...
  const referenceByStrategy: Record<string, keyof typeof configuration> = {
    AREA: "areaId",
...
export const validateDesignerGraph = (
  input: WorkflowDesignerSaveInput,
  processType?: string,
): WorkflowDesignerValidationResult => {
  const result: WorkflowDesignerValidationResult = {
    errors: [],
    isValid: false,
    warnings: [],
  };
  const nodesByReference = new Map<string, WorkflowNodeSaveInput>();
  const nodeReferences = new Map<WorkflowNodeSaveInput, string>();
  ...
  for (const node of input.nodes) {
    ...
    if (
      node.configurationJson.nodeType === "REJECTION" &&
  ...
  const incomingCount = new Map<WorkflowNodeSaveInput, number>();
  const outgoingCount = new Map<WorkflowNodeSaveInput, number>();
...
export const nextWorkflowVersionNumber = (versionNumbers: number[]): number => {
  return Math.max(0, ...versionNumbers) + 1;
};

export const summarizeWorkflowStatuses = (
...
const getWorkflowVersionForClone = async (
  db: WorkflowDatabase,
  workflowId: string,
...
const getNextVersionNumber = async (
  ...
  return nextWorkflowVersionNumber(
...
export const buildWorkflowAuditEvents = ({
  access,
  action,
  entityId,
  entityType,
...
const writeWorkflowAudit = async ({
  db,
  ...input
}: WorkflowAuditEventInput & { db: WorkflowDatabase }): Promise<void> => {
...
const cloneVersionContents = async (
  db: WorkflowDatabase,
  source: Awaited<ReturnType<typeof getWorkflowVersionForClone>>,
  ...
  const conditionGroupIdBySourceId = new Map<string, string>();
  ...
  for (const sourceTransition of source.transitions) {
    ...
    if (!sourceNodeId || !targetNodeId) {
      throw new AppError(
        "No se pudo duplicar una transición porque uno de sus nodos no existe.",
        409,
      );
    }
...
const createDraftVersionFromSource = async (
  db: WorkflowDatabase,
  source: Awaited<ReturnType<typeof getWorkflowVersionForClone>>,
  workflowId: string,
...
export const buildDefinitionOrderBy = (
  sortBy: ListWorkflowsQuery["sortBy"],
  sortDirection: ListWorkflowsQuery["sortDirection"],
...
export const buildDefinitionWhere = (
...
const mapPersistedDesignerInput = (
  version: WorkflowVersionDetailSelected,
): WorkflowDesignerSaveInput => {
  const nodes: WorkflowNodeSaveInput[] = [];

  for (const node of version.nodes) {
    const parsedConfiguration = workflowNodeConfigurationSchema.safeParse(
      node.configurationJson,
    );

    if (!parsedConfiguration.success) {
      throw new AppError(
        `La configuración del nodo ${node.nodeKey} no es válida.`,
        409,
      );
    }
    ...
    nodes.push({
      assignmentStrategy:
        node.assignmentStrategy === "MANAGEMENT"
          ? null
  ...
  const outgoingTransitionCount = new Map<string, number>();
  for (const transition of version.transitions) {
    outgoingTransitionCount.set(
      transition.sourceNode.id,
      (outgoingTransitionCount.get(transition.sourceNode.id) ?? 0) + 1,
    );
  }
  ...
  return {
    ...
    transitions: version.transitions.map((transition) => {
      const sourceNode = nodes.find(
        (node) => node.id === transition.sourceNode.id,
      );
      const legacySingleConditionRoute = Boolean(
      ...
      return {
        conditionGroup:
          transition.conditionGroup === null
            ? legacySingleConditionRoute &&
              sourceNode?.configurationJson.nodeType === "CONDITION"
              ? {
                  conditions: sourceNode.configurationJson.rules.map(
                    (rule, index) => ({
                      ...rule,
                      description: null,
                      sequence: index,
            ...
            : {
                conditions: transition.conditionGroup.conditions.map(
                  (condition) => ({
                    description: condition.description,
                    field:
                      canonicalizeWorkflowConditionField(condition.field) ??
                      (condition.field as (typeof WORKFLOW_CONDITION_FIELD_VALUES)[number]),
                    operator:
                      condition.operator as (typeof WORKFLOW_CONDITION_OPERATOR_VALUES)[number],
                    resultLabel: null,
                    sequence: condition.sequence,
                    value: condition.valueJson as never,
                  }),
                ),
        ...
        transitionType:
          transition.transitionType &&
          (WORKFLOW_TRANSITION_TYPE_VALUES as readonly string[]).includes(
            transition.transitionType,
          )
            ? (transition.transitionType as (typeof WORKFLOW_TRANSITION_TYPE_VALUES)[number])
            : legacySingleConditionRoute
              ? "CONDITION"
              : null,
      };
...
const buildDesignerResult = (
  version: WorkflowVersionDetailSelected,
  access: WorkflowActorContext,
) => {
  if (version.definition.status === "ARCHIVED") {
    throw new AppError(
      "No se puede abrir el diseñador de un workflow archivado.",
      409,
    );
  }
  ...
  return {
    ...
    lastSavedAt: new Date(
      Math.max(...savedDates.map((date) => date.getTime())),
    ),
    nodes: version.nodes,
...
const getDesignerVersion = async (
  db: WorkflowDatabase,
  versionId: string,
): Promise<WorkflowVersionDetailSelected> => {
...
const getWorkflowValidatorReferenceData = async (db: WorkflowDatabase) => {
...
const assertPersistableDesignerGraph = (
  input: WorkflowDesignerSaveInput,
  processType: string,
): void => {
  ...
  const blockingIssue = validation.errors.find((issue) =>
...
const invalidWorkflowValidationResult = (
  message: string,
): WorkflowValidationResult => ({
  errors: [
    {
...
const copyConditionGroupData = (
  conditionGroup: NonNullable<WorkflowTransitionSaveInput["conditionGroup"]>,
) => ({
    ...
    create: conditionGroup.conditions.map((condition, index) => ({
      ...
      valueJson:
        condition.value === undefined || condition.value === null
          ? Prisma.JsonNull
          : toInputJsonValue(condition.value as Prisma.JsonValue),
    })),
  },
});

const getConditionGroupForTransition = (
  transition: WorkflowTransitionSaveInput,
  sourceNode: WorkflowNodeSaveInput,
  ...
  if (transition.conditionGroupId) {
    ...
    return {
      ...
      id: existingGroup.id,
      logicOperator: existingGroup.logicOperator as "AND" | "OR",
    };
  }
...
const persistDesignerGraph = async (
  db: WorkflowDatabase,
  version: WorkflowVersionDetailSelected,
  input: WorkflowDesignerSaveInput,
): Promise<{
  ...
  if (
    referencedNodesOutsideVersion.some(
      (node) => node.workflowVersionId !== version.id,
    )
  ) {
    throw new AppError(
      "El grafo contiene nodos pertenecientes a otra versión.",
      400,
    );
  }
  ...
  const sourceNodeByPersistedId = new Map<string, WorkflowNodeSaveInput>();

  for (const node of input.nodes) {
    const existing =
      (node.id && isUuid(node.id)
        ? existingNodeById.get(node.id)
        : undefined) ?? existingNodeByKey.get(node.nodeKey);
    const configurationAssignmentStrategy =
      "assignmentStrategy" in node.configurationJson
        ? node.configurationJson.assignmentStrategy
        : null;
    const assignmentStrategy =
      node.assignmentStrategy ?? configurationAssignmentStrategy ?? null;
    ...
    if (existing) {
      const updated = await db.workflowNode.update({
        data: {
          assignmentStrategy,
          configurationJson: toInputJsonValue(
            node.configurationJson as unknown as Prisma.JsonValue,
          ),
  ...
  const incomingTransitionSourceIds = input.transitions.flatMap(
    (transition) => [transition.sourceNodeId, transition.targetNodeId],
  );
  for (const reference of incomingTransitionSourceIds) {
    if (!nodeIdByReference.has(reference)) {
      throw new AppError(
        "Una conexión referencia un nodo inexistente o de otra versión.",
        400,
      );
    }
  }
  ...
  for (const transition of input.transitions) {
    ...
    if (!sourceNodeId || !targetNodeId) {
      throw new AppError(
        "No se pudo resolver una conexión del diseñador.",
        400,
      );
    }
    ...
    if (!sourceNode) {
      throw new AppError(
        "No se pudo resolver el nodo origen de una conexión.",
        400,
      );
    }
    const conditionGroup = getConditionGroupForTransition(
      transition,
      sourceNode,
      input.transitions.filter(
        (candidate) => candidate.sourceNodeId === transition.sourceNodeId,
      ).length,
      existingGroupsById,
    );
...
export const workflowService = {
  ...
  async getWorkflowSummary(access: WorkflowActorContext) {
    assertPermission(access, WORKFLOW_PERMISSIONS.view);
  ...
  async getWorkflowOptions(access: WorkflowActorContext) {
  ...
  async getWorkflowDesignerOptions(access: WorkflowActorContext) {
    assertAnyPermission(access, [
    ...
    return {
      ...
      conditionFields: WORKFLOW_CONDITION_FIELD_VALUES.map((key) => ({
        key,
        label:
          WORKFLOW_RULE_FIELDS[key]?.label ??
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (
        ...
        if (previous._count.instances > 0 || publishedVersionCount > 0) {
          throw new AppError(
            "No se puede cambiar el proceso de un workflow publicado o con instancias.",
            409,
          );
        }
      }
  ...
  async archiveWorkflow(workflowId: string, access: WorkflowActorContext) {
  ...
  async duplicateWorkflow(
    ...
    return prisma.$transaction(async (db) => {
      ...
      const sourceVersion = await getWorkflowVersionForClone(
        db,
        workflowId,
        input.sourceVersionId,
      );
      const duplicateName =
        input.name ?? `Copia de ${source.name}`.slice(0, 191);
      ...
      const version = await createDraftVersionFromSource(
        db,
        sourceVersion,
        duplicate.id,
        1,
        access.userId,
        normalizeOptionalText(input.versionNotes) ??
  ...
  async createDraftVersion(
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (definition.status === "ARCHIVED") {
        throw new AppError(
          "No se pueden crear versiones en un workflow archivado.",
          409,
        );
      }

      const source = await getWorkflowVersionForClone(
        db,
        workflowId,
        input.sourceVersionId,
      );
      ...
      const version = await createDraftVersionFromSource(
        ...
        versionNumber,
        access.userId,
        normalizeOptionalText(input.changeDescription),
      );
  ...
  async getWorkflowVersion(versionId: string, access: WorkflowActorContext) {
    assertAnyPermission(access, [
      WORKFLOW_PERMISSIONS.simulate,
      WORKFLOW_PERMISSIONS.viewVersions,
    ]);
  ...
  async getWorkflowDesigner(versionId: string, access: WorkflowActorContext) {
    assertAnyPermission(access, [
      WORKFLOW_PERMISSIONS.edit,
      WORKFLOW_PERMISSIONS.simulate,
      WORKFLOW_PERMISSIONS.validate,
    ...
    return buildDesignerResult(
      await getDesignerVersion(prisma, versionId),
      access,
    );
  },
  ...
  async saveWorkflowDesigner(
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (version.definition.status === "ARCHIVED") {
        throw new AppError(
          "No se puede guardar el diseñador de un workflow archivado.",
          409,
        );
      }
      assertDraftWorkflowVersion(version.status);
      assertPersistableDesignerGraph(
        parsedInput,
        version.definition.processType,
      );
  ...
  async validateWorkflowDesigner(
    versionId: string,
    input: WorkflowDesignerValidateInput,
    access: WorkflowActorContext,
  ) {
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (version.definition.status === "ARCHIVED") {
        throw new AppError(
          "No se puede validar el diseñador de un workflow archivado.",
          409,
        );
      }
      assertDraftWorkflowVersion(version.status);
      ...
      try {
        const parsed = workflowDesignerSaveSchema.safeParse(
          input.graph ?? mapPersistedDesignerInput(version),
        );
        if (!parsed.success) {
          validation = invalidWorkflowValidationResult(
          ...
          validation = validateWorkflowGraph(parsed.data, {
            ...
            forPublication: true,
            knownConditionGroupIds: new Set(
              version.conditionGroups.map((group) => group.id),
            ),
  ...
  async simulateWorkflowVersion(
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (
        ...
        throw new AppError(
          "Las versiones archivadas no pueden simularse.",
          409,
        );
      }
      ...
      const validation = parsedGraph.success
        ...
        : invalidWorkflowValidationResult(
            "Una o más configuraciones de nodo no son válidas.",
          );
      ...
      const simulationContext = buildWorkflowSimulationContext({
        ...input.context,
        processType:
          input.context.processType ?? version.definition.processType,
      ...
      const result = simulateWorkflowGraph({
        ...
        request: {
          assignmentLookup: {
            areaManagerUserIds: Object.fromEntries(
              areaRows.map((area) => [area.id, area.managerUserId]),
            ),
          },
  ...
  async publishWorkflowVersion(
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (!parsedGraph.success) {
        throw new AppError(
          "El grafo contiene configuraciones inválidas y no puede publicarse.",
          409,
        );
      }
      ...
      if (!validation.isValid) {
        throw new AppError(
          `No se puede publicar el flujo: ${validation.errors.length} error(es) de validación.`,
          409,
          {
      ...
      const graphHash =
        ...
        buildWorkflowGraphHash({
          definitionId: version.definition.id,
          graph: parsedGraph.data as WorkflowGraph,
          processType: version.definition.processType,
          versionNumber: version.versionNumber,
        });
      if (input.graphHash !== graphHash) {
        throw new AppError(
          "El grafo cambió después de la última validación. Vuelva a validar antes de publicar.",
          409,
          { currentGraphHash: graphHash, validatedGraphHash: input.graphHash },
        );
      }
      ...
      await writeWorkflowAudit({
        ...
        newValues: {
          ...
          previousActiveVersion:
            publication.previousVersion?.versionNumber ?? null,
          publishedAt,
          warningCount: validation.warnings.length,
        },
  ...
  assertCanCancelInstance(access: AuthorizationSummary): void {
    assertPermission(access, WORKFLOW_INSTANCE_PERMISSIONS.cancel);
  },

  assertCanRetryInstance(access: AuthorizationSummary): void {
    assertPermission(access, WORKFLOW_INSTANCE_PERMISSIONS.retry);
  },

  assertCanPublishVersion(access: AuthorizationSummary): void {
    assertCanPublishWorkflowVersion(access);
  },
};
