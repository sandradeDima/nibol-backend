...
export type WorkflowAssignmentConfiguration = {
  ...
  assignmentStrategy?: string | null | undefined;
  ...
  fieldReference?: string | null | undefined;
  roleId?: string | null | undefined;
  userId?: string | null | undefined;
};

export type WorkflowAssignmentLookup = {
  areaManagerUserIds?: Record<string, string | null | undefined>;
};

export type WorkflowAssignmentProjection = {
  areaId?: string;
  fallbackApplied: boolean;
  kind: "AREA" | "ROLE" | "UNRESOLVED" | "USER";
  roleId?: string;
  strategy: string;
  userId?: string;
  warnings: string[];
...
export const buildWorkflowSimulationContext = (
  context: WorkflowSimulationContext,
): WorkflowSimulationContext => normalizeWorkflowSimulationContext(context);
...
const getContextReference = (
  ...
  if (field) return cleanReference(context[field]);
...
const baseProjection = (
  configuration: WorkflowAssignmentConfiguration,
): WorkflowAssignmentProjection => ({
...
export const resolveAssignmentProjection = (
  configuration: WorkflowAssignmentConfiguration,
  context: WorkflowSimulationContext,
  lookup: WorkflowAssignmentLookup = {},
): WorkflowAssignmentProjection => {
  ...
  switch (strategy) {
    ...
    case "RECORD_OWNER":
      projection.warnings.push(
        "El propietario del registro no está disponible en este contexto de simulación.",
      );
      break;
    ...
    case "FIELD_REFERENCE": {
      const referencedValue = getContextReference(
        context,
        configuration.fieldReference,
      );
      if (referencedValue) {
        projection.kind = "USER";
    ...
    case "MANAGEMENT":
      projection.errors.push(
        "La estrategia MANAGEMENT no está disponible en esta fase.",
      );
      break;
    case null:
    case undefined:
  ...
  switch (configuration.fallbackStrategy) {
    ...
    case "USER":
      if (configuration.fallbackUserId) {
        ...
        projection.warnings.push(
          "Se aplicó el usuario de respaldo configurado.",
        );
        return projection;
      }
      break;
    case "ADMINISTRATOR":
      projection.fallbackApplied = true;
      projection.warnings.push(
        "Se proyectó una escalada al administrador; no se resolvió un usuario concreto.",
      );
      return projection;
    case "STOP":
    case null:
  ...
  projection.errors.push(
    "No se pudo resolver un responsable concreto con la configuración disponible.",
  );
  return projection;
};

export const conditionFieldForReference = (
  reference: string | null | undefined,
