...
const matches = (field: string, operator: string, value?: unknown): boolean =>
  evaluateWorkflowRule(rule(field, operator, value), context, { now }).matched;
...
test("evalúa vencimiento y ventana de fecha usando el reloj recibido", () => {
  ...
  assert.equal(
    evaluateWorkflowRule(
      rule("dueDate", "IS_OVERDUE"),
      { ...context, dueDate: "2026-08-03" },
      { now },
    ).matched,
    true,
  );
...
test("rechaza campos u operadores incompatibles antes de evaluar", () => {
  assert.match(
    validateWorkflowRule(rule("riskLevel", "GREATER_THAN", "HIGH")) ?? "",
    /no es compatible/,
  );
  assert.match(
    evaluateWorkflowRule(rule("unknownField", "EQUALS", "x"), context, { now })
      .message,
    /no está soportado/,
  );
});
...
test("evalúa grupos AND y OR completos en secuencia estable", () => {
  const andResult = evaluateConditionGroup(
    ...
    context,
    { detailed: true, now, shortCircuit: false },
  );
  ...
  assert.deepEqual(
    andResult.results.map((result) => result.conditionId),
    ["evidenceCount-GREATER_THAN", "riskLevel-EQUALS"],
  );
