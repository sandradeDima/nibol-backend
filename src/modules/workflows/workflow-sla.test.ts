...
test("rechaza duraciones no positivas y conserva instantes UTC", () => {
  assert.throws(
    () => calculateWorkflowDeadline({ duration: 0, startedAt, unit: "HOURS" }),
    /entero positivo/,
  );
  assert.throws(
    () =>
      calculateWorkflowDeadline({ duration: -1, startedAt, unit: "MINUTES" }),
    /entero positivo/,
  );
  assert.equal(
    calculateWorkflowDeadline({
...
test("los umbrales nulos no crean una fecha y el estado SLA es determinista", () => {
  assert.equal(
    calculateWorkflowThresholdAt({ startedAt, threshold: null, unit: "HOURS" }),
    null,
  );
  ...
  assert.equal(
    getWorkflowSlaState({ dueAt, now: new Date("2026-08-07T15:00:00.000Z") }),
    "OVERDUE",
  );
