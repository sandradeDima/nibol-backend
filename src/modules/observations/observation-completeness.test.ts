...
import {
  buildObservationActionItems,
  buildObservationActionSummary,
  type ObservationActionContext,
} from "./observation-completeness.service.js";
...
const context = (): ObservationActionContext => ({
  actionPlans: [],
  areaAssignments: [
    {
...
test("completeness identifies missing action plan and finding evidence", () => {
  const items = buildObservationActionItems(
    context(),
    access,
...
test("overdue action points to the plan tab", () => {
  const items = buildObservationActionItems(
    { ...context(), currentDueDate: new Date("2026-08-01T00:00:00.000Z") },
    access,
    new Date("2026-08-14T00:00:00.000Z"),
  );
