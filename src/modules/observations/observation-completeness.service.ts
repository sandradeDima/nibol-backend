...
export type ObservationActionItem = {
  actionLabel?: string;
  actionType?: string;
  actionUrl?: string;
  areaId?: string;
...
export type ObservationActionSummary = {
  count: number;
  items: ObservationActionItem[];
  primaryAction: ObservationActionItem | null;
  severity: ObservationActionSeverity | "NONE";
  status: "ATTENTION" | "COMPLETE" | "OVERDUE";
};

export type ObservationAttentionFilter =
  | "DUE_SOON"
  | "HAS_PENDING"
  | "MISSING_EVIDENCE"
  | "MISSING_PLAN"
...
export type ObservationActionObservation = {
  areaAssignments: Array<{
    areaId: string;
    areaName: string;
    areaResponsibleUserId: string | null;
...
export type ObservationActionContext = ObservationActionObservation & {
  actionPlans: Array<{
    currentDueDate: Date;
    id: string;
...
const hasPermission = (
  ...
  permission: string,
): boolean => access.isAdmin || access.permissions.includes(permission);

const actionable = (
  item: ObservationActionItem,
  access: AuthorizationSummary,
): ObservationActionItem => {
  if (!item.permission || hasPermission(access, item.permission)) return item;
  const result = { ...item };
...
export const buildObservationAttentionWhere = (
  attention: ObservationAttentionFilter,
  dueSoonDays = 7,
  now = new Date(),
...
export const buildObservationActionItems = (
  context: ObservationActionContext,
  access: AuthorizationSummary,
  now = new Date(),
): ObservationActionItem[] => {
  if (context.status.isFinal) return [];
  const items: ObservationActionItem[] = [];
...
export const buildObservationActionSummary = (
  items: ObservationActionItem[],
): ObservationActionSummary => {
  const rank = { CRITICAL: 3, INFO: 1, WARNING: 2 } as const;
  const severity = items.reduce<ObservationActionSeverity | "NONE">(
    (current, item) =>
      current === "NONE" || rank[item.severity] > rank[current]
        ? item.severity
        : current,
    "NONE",
  );
...
const loadContexts = async (
  observations: ObservationActionObservation[],
): Promise<Map<string, ObservationActionContext>> => {
  ...
  const evidenceCounts = new Map(
    findingEvidence.map((row) => [row.observationId, row._count._all]),
  );
  const pendingCounts = new Map<string, number>();
  for (const row of pendingReviews) {
    pendingCounts.set(
      row.actionPlan.observationId,
      (pendingCounts.get(row.actionPlan.observationId) ?? 0) + 1,
    );
  }
  return new Map(
    observations.map((observation) => [
      observation.id,
      {
        ...
        actionPlans: actionPlans.filter(
          (actionPlan) => actionPlan.observationId === observation.id,
        ),
...
export const observationCompletenessService = {
  async getForObservation(
    observation: ObservationActionObservation,
    access: AuthorizationSummary,
  ...
  async getSummaries(
    observations: ObservationActionObservation[],
    access: AuthorizationSummary,
  ): Promise<Map<string, ObservationActionSummary>> {
