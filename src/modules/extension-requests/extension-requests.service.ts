...
import {
  ACTIVE_EXTENSION_REQUEST_STATUSES,
  EDITABLE_EXTENSION_REQUEST_STATUSES,
  FINAL_EXTENSION_REQUEST_STATUSES,
} from "./extension-requests.constants.js";
import type {
  CreateExtensionRequestInput,
  DeadlineExtensionCommitmentSummary,
  DeadlineExtensionEvidenceItem,
  DeadlineExtensionRequestDetail,
  DeadlineExtensionRequestListItem,
  DeadlineExtensionRequestMutationResult,
  DeadlineExtensionStatusValue,
  DeadlineExtensionUserSummary,
  ListExtensionRequestsQuery,
  ReviewExtensionRequestInput,
  UpdateExtensionRequestInput,
} from "./extension-requests.types.js";

const extensionRequestPrisma = prisma as typeof prisma & {
  commitment: any;
  deadlineExtensionAttachment: any;
...
const cancellableStatuses = new Set<DeadlineExtensionStatusValue>([
  "DRAFT",
  "SENT_TO_MANAGER",
  "SENT_TO_AUDIT",
]);
...
): DeadlineExtensionUserSummary | null => {
  if (!user) {
    return null;
  }
...
const buildObservationEffectiveStatus = (
  observation: any,
): {
  key: string;
  name: string;
...
const isCommitmentCompleted = (commitment: any): boolean => {
  return (
...
const isCommitmentOverdue = (commitment: any): boolean => {
  return !isCommitmentCompleted(commitment) && commitment.dueDate.getTime() < Date.now();
};

const buildCommitmentEffectiveStatus = (
  commitment: any,
): {
  key: string;
  name: string;
...
const calculateImpactDays = (currentDueDate: Date, requestedDueDate: Date): number => {
  return Math.round(
    (requestedDueDate.getTime() - currentDueDate.getTime()) / (1000 * 60 * 60 * 24),
  );
};

const parseBooleanParameterValue = (
...
const getApprovalSettings = async (): Promise<ApprovalSettings> => {
  ...
  const values = new Map<string, string>(
    parameters.map((parameter: { key: string; value: string }) => [
      parameter.key,
      parameter.value,
    ]),
  );
  ...
  return {
    allowRequests: parseBooleanParameterValue(
      values.get("allow_deadline_extension"),
      true,
    ),
    requiresAuditApproval: parseBooleanParameterValue(
      values.get("extension_requires_audit_approval"),
      true,
    ),
    requiresManagerApproval: parseBooleanParameterValue(
      values.get("extension_requires_manager_approval"),
      true,
    ),
  };
};

const ensureDeadlineExtensionsEnabled = (settings: ApprovalSettings): void => {
  if (!settings.allowRequests) {
    throw new AppError(
      "La solicitud de ampliaciones de plazo se encuentra deshabilitada en los parametros del sistema.",
      403,
    );
  }
};

const buildAssignedAreas = (observation: any) => {
  return [
    {
    ...
    ...observation.areaAssignments.map((assignment: any) => ({
...
const resolveObservationRequesterArea = (
  observation: any,
  ...
  return (
    buildAssignedAreas(observation).find((areaContext: any) => {
      return (
...
const canRequestCommitmentExtension = (
  commitment: any,
  access: AuthorizationSummary,
): boolean => {
  if (isSystemWideAccess(access)) {
  ...
  return (
    commitment.remediationPlan.area.managerUser?.id === access.userId ||
    commitment.responsibleUserId === access.userId ||
    commitment.remediationPlan.ownerUser?.id === access.userId
  );
};

const canViewRequest = (request: any, access: AuthorizationSummary): boolean => {
  ...
  if (
    request.area.managerUser?.id === access.userId ||
  ...
  return request.observation.areaAssignments.some((assignment: any) => {
    return (
      assignment.responsibleUser?.id === access.userId ||
      assignment.area.managerUser?.id === access.userId
    );
  });
};

const canReviewRequestAsManager = (
  request: any,
  access: AuthorizationSummary,
): boolean => {
...
const canEditRequest = (request: any, access: AuthorizationSummary): boolean => {
...
const canCancelRequest = (request: any, access: AuthorizationSummary): boolean => {
...
const ensureReviewComment = (
  action: "audit-reject" | "manager-reject",
  ...
  if (comment.length < 3) {
    throw new AppError(
      action === "manager-reject"
        ...
        : "Debe registrar un comentario para rechazar la solicitud como Auditoria.",
      400,
    );
  }
...
const ensureRequestedDueDateAfterCurrent = (
  requestedDueDate: Date,
  currentDueDate: Date,
): void => {
  if (requestedDueDate.getTime() <= currentDueDate.getTime()) {
    throw new AppError(
      "La nueva fecha solicitada debe ser posterior a la fecha limite actual.",
      400,
    );
  }
};

const ensureObservationEligibleForExtension = (observation: any): void => {
  if (terminalObservationStatuses.has(observation.status.key)) {
    throw new AppError(
      "No puede solicitar ampliaciones de plazo para observaciones cerradas o terminales.",
      409,
    );
  }
};

const ensureCommitmentEligibleForExtension = (commitment: any): void => {
  if (isCommitmentCompleted(commitment)) {
    throw new AppError(
      "No puede solicitar ampliaciones de plazo para compromisos completados.",
      409,
    );
  }
};
...
const assertNoActiveRequest = async (input: {
  ...
  if (activeRequest) {
    throw new AppError(
      "Ya existe una solicitud activa de ampliacion de plazo para este objetivo.",
      409,
    );
  }
};

const findObservationContextById = async (
  observationId: string,
): Promise<any> => {
...
const findCommitmentContextById = async (commitmentId: string): Promise<any> => {
...
const getRequestContextById = async (requestId: string): Promise<any> => {
...
const findRequestContextById = async (
  requestId: string,
  access: AuthorizationSummary,
): Promise<any> => {
...
const syncRequestAttachments = async (
  requestId: string,
  observationId: string,
  evidenceFileIds: string[],
  db: typeof extensionRequestPrisma,
): Promise<void> => {
  ...
  if (evidenceFiles.length !== uniqueEvidenceFileIds.length) {
    throw new AppError(
      "Uno o mas documentos de respaldo no pertenecen a la observacion seleccionada.",
      400,
    );
  }
...
const assertTargetDueDateIsCurrent = (request: any): void => {
  ...
  if (currentTargetDueDate.getTime() !== request.currentDueDate.getTime()) {
    throw new AppError(
      "La fecha limite actual del objetivo cambio. Actualice la solicitud antes de continuar.",
      409,
    );
  }
...
const updateTargetDueDate = async (
  request: any,
  db: typeof extensionRequestPrisma,
): Promise<void> => {
...
const buildDueDateStakeholderUserIds = (request: any): string[] => {
  return Array.from(
    new Set(
      [
        ...
        ...request.observation.areaAssignments.flatMap((assignment: any) => [
...
const notifySentToManager = async (request: any, actorUserId: string): Promise<void> => {
...
const notifySentToAudit = async (request: any, actorUserId: string): Promise<void> => {
...
const notifyRejectedToRequester = async (
  request: any,
  title: string,
  message: string,
): Promise<void> => {
...
const notifyFinalApproval = async (request: any, actorUserId: string): Promise<void> => {
  const userIds = buildDueDateStakeholderUserIds(request).filter(
...
const mapEvidenceItem = (evidence: any): DeadlineExtensionEvidenceItem => {
  return {
...
const mapCommitmentSummary = (
  commitment: any,
): DeadlineExtensionCommitmentSummary | null => {
  if (!commitment) {
    return null;
  }
...
const mapRequestListItem = (
  ...
  return {
    ...
    canReview:
      canReviewRequestAsAudit(request, access) || canReviewRequestAsManager(request, access),
...
const mapRequestDetail = (
  ...
  return {
    ...
    attachments: request.attachments.map((attachment: any) =>
      mapEvidenceItem(attachment.evidenceFile),
    ),
...
const buildListWhere = (
  ...
  const requestedDueDate = buildDateRangeFilter(
    query.requestedDateFrom,
    query.requestedDateTo,
  );
...
const buildListOrderBy = (
  sortBy: ListExtensionRequestsQuery["sortBy"],
  sortDirection: ListExtensionRequestsQuery["sortDirection"],
): any => {
  switch (sortBy) {
    case "currentDueDate":
      return {
...
const buildMutationResult = async (
  requestId: string,
  access: AuthorizationSummary,
  previousRequest: any | null,
): Promise<DeadlineExtensionRequestMutationResult> => {
...
const createRequest = async (input: {
  ...
  const requestId = await extensionRequestPrisma.$transaction(async (transaction) => {
    ...
    await syncRequestAttachments(
      created.id,
      input.observationId,
      input.payload.evidenceFileIds ?? [],
      transaction as typeof extensionRequestPrisma,
    );
...
export const extensionRequestsService = {
  async auditApprove(
    ...
    await extensionRequestPrisma.$transaction(async (transaction) => {
      await updateTargetDueDate(request, transaction as typeof extensionRequestPrisma);
  ...
  async auditReject(
    ...
    const result = await buildMutationResult(requestId, access, request);
    await notifyRejectedToRequester(
      await getRequestContextById(requestId),
      "Ampliacion rechazada por Auditoria",
      `Auditoria rechazo la solicitud de ampliacion para ${request.observation.code}. Revise el comentario y ajuste la propuesta.`,
    );
    return result;
  },
  ...
  async createForCommitment(
    ...
    if (!canRequestCommitmentExtension(commitment, access)) {
      throw new AppError(
        "No puede solicitar ampliaciones para este compromiso.",
        403,
      );
    }
  ...
  async createForObservation(
    ...
    if (!requesterArea) {
      throw new AppError(
        "Solo el area asignada o su gerencia puede solicitar ampliaciones para esta observacion.",
        403,
      );
    }
  ...
  async getById(
    requestId: string,
    access: AuthorizationSummary,
  ): Promise<DeadlineExtensionRequestDetail> {
    const settings = await getApprovalSettings();
    const request = await findRequestContextById(requestId, access);
  ...
  async list(
    ...
    data: DeadlineExtensionRequestListItem[];
    pagination: {
      page: number;
      perPage: number;
      total: number;
    ...
    const where = buildListWhere(query, access) as Prisma.DeadlineExtensionRequestWhereInput;
    ...
    return {
      data: requests.map((request: any) => mapRequestListItem(request, access)),
      pagination: {
        page: query.page,
  ...
  async managerReject(
    ...
    await notifyRejectedToRequester(
      await getRequestContextById(requestId),
      "Ampliacion rechazada por Gerencia",
      `Gerencia rechazo la solicitud de ampliacion para ${request.observation.code}. Revise el comentario y ajuste la propuesta.`,
    );
    return result;
  },
  ...
  async sendToAudit(
    ...
    if (settings.requiresManagerApproval && request.status !== "MANAGER_APPROVED") {
      throw new AppError(
        "Esta solicitud requiere aprobación previa de Gerencia antes de enviarse a Auditoria.",
        409,
      );
    }
  ...
  async sendToManager(
    ...
    if (!settings.requiresManagerApproval) {
      throw new AppError(
        "La aprobación de Gerencia no es requerida para esta solicitud. Envíela directamente a Auditoria.",
        409,
      );
    }
