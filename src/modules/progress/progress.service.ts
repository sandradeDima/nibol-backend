...
import {
  AREA_VISIBLE_COMMENT_VISIBILITIES,
  AUDIT_VISIBLE_COMMENT_VISIBILITIES,
  EDITABLE_PROGRESS_STATUSES,
} from "./progress.constants.js";
import type {
  CommentVisibilityValue,
  CreateCommentInput,
  CreateProgressUpdateInput,
  EvidenceFileItem,
  ListProgressUpdatesQuery,
  ObservationCommentItem,
  ObservationProgressListItem,
  ObservationProgressWorkspace,
  ProgressAreaSummary,
  ProgressCommitmentTarget,
  ProgressPlanTarget,
  ProgressReviewActionValue,
  ProgressReviewHistoryEntry,
  ProgressUpdateItem,
  ProgressUserSummary,
...
const progressPrisma = prisma as typeof prisma & {
  commitment: any;
  evidenceFile: any;
  observation: any;
  observationComment: any;
...
type ObservationContextRow = any;
type AreaContext = {
  area: {
    id: string;
...
const mapUserSummary = (user: any | null | undefined): ProgressUserSummary | null => {
  if (!user) {
    return null;
  }
...
const canManageObservationProgress = (
  observation: ObservationContextRow,
  access: AuthorizationSummary,
): boolean => {
...
const canContributeToObservation = (
  observation: ObservationContextRow,
  access: AuthorizationSummary,
  ...
  return observation.commitments.some((commitment: any) => {
    return commitment.id === commitmentId && commitment.responsibleUserId === access.userId;
  });
};

const canInteractWithObservation = (
  observation: ObservationContextRow,
  access: AuthorizationSummary,
): boolean => {
  ...
  return observation.commitments.some((commitment: any) => commitment.responsibleUserId === access.userId);
};
...
const getCommentVisibilityValues = (
  access: AuthorizationSummary,
): Set<CommentVisibilityValue> => {
  if (isSystemWideAccess(access) || hasAuditReviewRole(access)) {
    return AUDIT_VISIBLE_COMMENT_VISIBILITIES as Set<CommentVisibilityValue>;
  }

  return AREA_VISIBLE_COMMENT_VISIBILITIES as Set<CommentVisibilityValue>;
};
...
const mapReviewHistoryEntry = (entry: any): ProgressReviewHistoryEntry => {
  return {
    action: entry.action,
...
const mapProgressUpdateItem = (
  ...
  return {
    ...
    evidences: progressUpdate.evidenceFiles.map((evidence: any) =>
      mapEvidenceItem(evidence, canEdit),
    ),
    history: progressUpdate.reviewHistory.map((entry: any) =>
      mapReviewHistoryEntry(entry),
    ),
...
const buildProgressListArea = (progressUpdate: any): ProgressAreaSummary => {
  return progressUpdate.remediationPlan?.area ?? progressUpdate.observation.area;
};

const buildProgressListResponsibleUser = (progressUpdate: any): ProgressUserSummary | null => {
  return (
    mapUserSummary(progressUpdate.commitment?.responsibleUser) ??
    mapUserSummary(progressUpdate.remediationPlan?.ownerUser) ??
    mapUserSummary(progressUpdate.observation.responsibleUser)
  );
};

const extractSentToAuditAt = (progressUpdate: any): string | null => {
  const lastSentEntry = [...progressUpdate.reviewHistory]
    .reverse()
    .find((entry: any) => entry.action === "SENT");

  return lastSentEntry?.createdAt?.toISOString?.() ?? null;
};

const buildProgressListWhere = (
  query: ListProgressUpdatesQuery,
  access: AuthorizationSummary,
): any => {
...
const buildProgressListOrderBy = (
  sortBy: ListProgressUpdatesQuery["sortBy"],
  sortDirection: ListProgressUpdatesQuery["sortDirection"],
): any => {
  switch (sortBy) {
...
const buildEvidenceAbsolutePath = (relativePath: string): string => {
  return path.join(uploadsRootDir, relativePath);
};

const buildEvidenceRelativePath = (date: Date, storedName: string): string => {
...
const getAllowedMimeTypesForExtension = (extension: string): Set<string> | null => {
  return (ALLOWED_EVIDENCE_TYPES as Record<string, Set<string>>)[extension] ?? null;
};

const assertEvidenceFileAllowed = (
  file: {
    buffer: Buffer;
    mimetype: string;
    originalName: string;
  ...
  if (file.size > maxFileSizeBytes) {
    throw new AppError(
      `El archivo supera el limite permitido de ${Math.round(maxFileSizeBytes / (1024 * 1024))} MB.`,
      400,
    );
  }
...
const getEvidenceMaxFileSizeBytes = async (): Promise<number> => {
...
const prepareEvidenceFiles = async (
  files: Array<{
    buffer: Buffer;
    mimetype: string;
    originalName: string;
...
const cleanupPreparedEvidenceFiles = async (
  files: PreparedEvidenceFile[],
): Promise<void> => {
  await Promise.all(
    files.map(async (file) => {
      try {
...
const resolveLinkedTargets = (
  ...
  if (remediationPlanId) {
    const plan = observation.remediationPlans.find((item: any) => item.id === remediationPlanId);
  ...
  if (commitmentId) {
    const commitment = observation.commitments.find((item: any) => item.id === commitmentId);
...
const findProgressUpdateContextById = async (
...
const findEvidenceContextById = async (
  evidenceId: string,
  access: AuthorizationSummary,
): Promise<any> => {
  const visibilityCondition = buildObservationVisibilityCondition(access);
...
const findCommentContextById = async (
  commentId: string,
  access: AuthorizationSummary,
): Promise<any> => {
  const visibilityCondition = buildObservationVisibilityCondition(access);
...
const recalculateObservationProgress = async (
  observationId: string,
  db: typeof progressPrisma = progressPrisma,
): Promise<number> => {
...
const maybeUpdateObservationStatusAfterApproval = async (
  observationId: string,
  input: {
  ...
  >(statuses.map((status: any) => [status.key, status]));
...
const buildStakeholderNotificationUserIds = (
  observation: ObservationContextRow,
  ...
  return Array.from(
    new Set(
      [
        ...
        ...observation.areaAssignments.flatMap((assignment: any) => [
...
const ensureReviewCommentForAction = (
  ...
  if (comment.length < 3) {
    throw new AppError(
      action === "return"
        ...
        : "Debe registrar un comentario para rechazar el avance.",
      400,
    );
  }
...
export const progressService = {
  async createObservationComment(
    ...
    let progressUpdateContext: any | null = null;
    ...
    if (
      requestedVisibility === "INTERNAL_AUDIT" &&
  ...
  async createProgressUpdateEvidence(
    progressUpdateId: string,
    files: Array<{
      buffer: Buffer;
      mimetype: string;
    ...
    input: UploadObservationEvidenceInput,
    access: AuthorizationSummary,
    ...
    try {
      ...
      return createdEvidence.map((evidence: any) => mapEvidenceItem(evidence, true));
    } catch (error) {
  ...
  async getObservationProgressWorkspace(
    ...
    return {
      ...
      commitments: observation.commitments
        ...
        .map(
          (commitment: any): ProgressCommitmentTarget => ({
            id: commitment.id,
      ...
      plans: observation.remediationPlans
        ...
        .map(
          (plan: any): ProgressPlanTarget => ({
            area: plan.area,
            id: plan.id,
            ownerUser: mapUserSummary(plan.ownerUser),
            responsibleUser:
              mapUserSummary(plan.ownerUser) ??
              mapUserSummary(observation.responsibleUser),
            status: plan.status,
          }),
        ),
      progressUpdates: progressUpdates.map((progressUpdate: any) =>
        mapProgressUpdateItem(progressUpdate, observation, access),
      ),
    };
  },
  ...
  async getObservationComments(
    ...
    return comments.map((comment: any) => mapCommentItem(comment, access));
  },
  ...
  async listProgressUpdates(
    ...
    data: ObservationProgressListItem[];
    pagination: {
      page: number;
      perPage: number;
      total: number;
    ...
    return {
      data: progressUpdates.map((progressUpdate: any) => {
        const responsibleUser = buildProgressListResponsibleUser(progressUpdate);
        const canEdit =
          EDITABLE_PROGRESS_STATUSES.has(progressUpdate.status) &&
        ...
        return {
          ...
          status: progressUpdate.status,
          type: progressUpdate.type,
        } satisfies ObservationProgressListItem;
      }),
      pagination: {
        page: query.page,
  ...
  async reviewProgressUpdate(
    ...
    const nextStatus =
      action === "approve"
        ...
        : action === "return"
          ? "RETURNED"
          : "REJECTED";
    const historyAction: ProgressReviewActionValue =
      action === "approve"
        ? "APPROVED"
        : action === "return"
          ? "RETURNED"
    ...
    await progressPrisma.$transaction(async (transaction) => {
      ...
      if (action === "approve") {
        await recalculateObservationProgress(observation.id, transaction as typeof progressPrisma);
        await maybeUpdateObservationStatusAfterApproval(
          observation.id,
          {
            progressPercent: progressUpdate.progressPercent,
            type: progressUpdate.type,
          },
          transaction as typeof progressPrisma,
        );
      }
  ...
  async sendProgressUpdateToAudit(
    progressUpdateId: string,
    access: AuthorizationSummary,
  ): Promise<ProgressUpdateItem> {
    const progressUpdate = await findProgressUpdateContextById(progressUpdateId, access);
  ...
  async updateObservationComment(
    ...
    if (
      input.visibility === "INTERNAL_AUDIT" &&
  ...
  async updateProgressUpdate(
    ...
    const linkedTargets = resolveLinkedTargets(observation, {
      commitmentId:
        input.commitmentId !== undefined ? input.commitmentId : progressUpdate.commitmentId,
      remediationPlanId:
        input.remediationPlanId !== undefined
