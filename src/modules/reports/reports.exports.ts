...
export const buildExcelWorkbook = ({
  ...
  const rowXml = (values: unknown[]) =>
    `<Row>${values.map((value) => `<Cell><Data ss:Type="String">${escapeXml(stringifyCell(value))}</Data></Cell>`).join("")}</Row>`;
...
const wrapText = (value: string, width = 105): string[] => {
...
export const buildSimplePdf = ({
  ...
  const lines = [
    ...
    columns.join(" | "),
    ...rows.flatMap((row) =>
      wrapText(columns.map((column) => stringifyCell(row[column])).join(" | ")),
    ),
  ];
  const pages: string[][] = [];
  ...
  if (pages.length === 0) pages.push(["Sin registros"]);
