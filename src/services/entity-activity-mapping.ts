export const getRemediationActivityType = (entityType: string, action: string): string => {
  if (entityType === "remediation_plan") {
    ...
    if (action.endsWith("send-to-audit")) return "PLAN_SENT_TO_AUDIT";
...
export const getProgressActivityType = (entityType: string, action: string): string => {
  if (entityType === "evidence_file") {
    ...
    if (action.endsWith("delete")) return "EVIDENCE_DELETED";
  }
  if (entityType === "observation_comment") {
    if (action.endsWith("create")) return "COMMENT_ADDED";
    if (action.endsWith("update")) return "COMMENT_EDITED";
    if (action.endsWith("delete")) return "COMMENT_DELETED";
  }
  if (action.endsWith("approve")) return "PROGRESS_APPROVED";
  if (action.endsWith("return")) return "PROGRESS_RETURNED";
...
export const getExtensionActivityType = (action: string, status?: string): string => {
  ...
  if (action.endsWith("send-to-audit")) return "EXTENSION_SENT_TO_AUDIT";
  if (action.endsWith("audit-approve")) return "EXTENSION_AUDIT_APPROVED";
  if (action.endsWith("audit-reject")) return "EXTENSION_AUDIT_REJECTED";
  if (action.endsWith("cancel")) return "EXTENSION_CANCELLED";
  if (status === "AUDIT_APPROVED") return "DEADLINE_UPDATED_BY_EXTENSION";
