...
import {
  ...
  DEADLINE_REMINDER_LOCK_NAME,
  DEADLINE_REMINDER_PARAMETER_DEFAULTS,
  DEADLINE_REMINDER_POLICY_DEFAULTS,
  DEADLINE_REMINDER_ROLE_LABELS,
  DEADLINE_REMINDER_ROLES,
  getDeadlineReminderParameterKey,
  type DeadlineReminderPolicy,
  type DeadlineReminderRole,
...
import {
  ...
  getReminderBucket,
  getScheduleGroupKey,
  getScheduledReminderPeriods,
  getNextScheduledReminderPeriod,
} from "./deadline-reminder.policy.js";
...
type StoredPolicy = DeadlineReminderPolicy & { createdAt: Date };
...
export type DeadlineReminderRunSummary = {
  emailsSent: number;
  executionIds: string[];
...
const dateAtNoonUtc = (dateKey: string): Date =>
...
const errorMessage = (error: unknown): string =>
  (error instanceof Error ? error.message : "Error desconocido").slice(0, 500);
...
const readPolicies = async (): Promise<StoredPolicy[]> => {
  ...
  return DEADLINE_REMINDER_ROLES.map((role) => {
    ...
    const enabledRecord = byKey.get(
      getDeadlineReminderParameterKey(role, "enabled"),
    );
    const cutoffRecord = byKey.get(
      getDeadlineReminderParameterKey(role, "cutoffDay"),
    );
    const cadenceRecord = byKey.get(
    ...
    return {
      cadenceMonths: Math.min(
        24,
        parsePositiveInteger(cadenceRecord?.value, defaults.cadenceMonths),
      ),
      cutoffDay: Math.min(
        28,
        Math.max(
          1,
          parsePositiveInteger(cutoffRecord?.value, defaults.cutoffDay),
        ),
      ),
      createdAt,
      enabled:
        enabledRecord?.active === false
          ? false
          : parseBoolean(enabledRecord?.value, defaults.enabled),
      role,
      upcomingWindowDays: Math.min(
        365,
        parsePositiveInteger(windowRecord?.value, defaults.upcomingWindowDays),
      ),
    };
  });
};
...
const readBusinessTimeZone = async (): Promise<string> => {
...
const toDigestPlan = (
  ...
  const approvedExtension = plan.deadlineExtensionRequests?.find(
    (extension: { status: string }) => extension.status === "MANAGER_APPROVED",
  );
  return {
...
const bucketOrder: Record<DigestPlan["bucket"], number> = {
  DUE_TODAY: 1,
  OVERDUE: 0,
  UPCOMING: 2,
};

const sortDigestPlans = (plans: DigestPlan[]): DigestPlan[] =>
  plans.sort(
    (left, right) =>
...
const buildDigests = async (input: {
...
}): Promise<RecipientDigest[]> => {
  const byRecipient = new Map<
    string,
    RecipientDigest & { planIds: Set<string>; rolePolicies: StoredPolicy[] }
  >();
  for (const policy of input.policies) {
    ...
    const showExecutor = input.policies.some(
      (item) => item.role !== "EXECUTOR",
    );
    for (const recipient of recipients) {
      const plans = await findActionPlans(
        policy,
        input.cutoffDateKey,
        recipient.id,
      );
      for (const plan of plans) {
        const digestPlan = toDigestPlan(
          plan,
          input.cutoffDateKey,
          policy,
          showExecutor,
        );
        if (!digestPlan) continue;
...
const notificationMessage = (digest: RecipientDigest): string => {
...
const deliverReminderEmailDelivery = async (
  ...
  const payload = delivery.payloadJson as unknown as ReminderPayload;
  try {
...
const acquireLock = async (): Promise<(() => Promise<void>) | null> => {
  const now = new Date();
...
const createExecution = async (input: {
  ...
  try {
    const data: Prisma.ScheduledJobExecutionUncheckedCreateInput = {
      ...
      triggeredBy: input.triggeredBy as ScheduledJobTrigger,
    ...
    if (
      existing.status === ScheduledJobExecutionStatus.SUCCESS ||
...
const notifyDigest = async (input: {
  channels: { notifyByEmail: boolean; notifyInApp: boolean };
  cutoffDateKey: string;
  digest: RecipientDigest;
  executionId: string;
  periodKey: string;
  roleCadence: string;
  roleCadenceKey: string;
  runType: string;
  ...
  if (input.channels.notifyInApp) {
    ...
    if (notificationResult.created) input.summary.notificationsCreated += 1;
  ...
  if (notified) input.summary.recipientsNotified += 1;
};
...
const executePeriod = async (input: {
  ...
  const summary: DeadlineReminderRunSummary = {
    emailsSent: 0,
...
const mergeSummaries = (
  target: DeadlineReminderRunSummary,
  source: DeadlineReminderRunSummary,
): void => {
...
export const deadlineReminderService = {
  async updatePolicy(
    role: DeadlineReminderRole,
    input: Omit<DeadlineReminderPolicy, "role">,
  ) {
    const previous = (await readPolicies()).find(
  ...
  async run(options: {
    ...
    const baseSummary: DeadlineReminderRunSummary = {
      emailsSent: 0,
      executionIds: [],
      failures: [],
      failuresCount: 0,
    ...
    try {
      ...
      if (options.mode === MANUAL_RUN) {
        const selected = policies.filter(
          (policy) => !options.role || policy.role === options.role,
        );
        const groups = new Map<string, StoredPolicy[]>();
        for (const policy of selected) {
          const key = getScheduleGroupKey(policy);
          groups.set(key, [...(groups.get(key) ?? []), policy]);
        }
        const cutoffDateKey =
          options.cutoffDateKey ?? getBusinessDateKey(startedAt, timeZone);
        ...
        for (const [cadenceKey, group] of groups) {
          ...
          const periods = getScheduledReminderPeriods({
            createdAt: group.reduce(
              (earliest, policy) =>
                policy.createdAt < earliest ? policy.createdAt : earliest,
              group[0]!.createdAt,
            ),
