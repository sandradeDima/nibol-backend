...
test("aplica los límites inclusivos de la ventana mensual", () => {
  ...
  assert.equal(
    getReminderBucket({ ...input, effectiveDueDateKey: "2026-09-14" }),
    "OVERDUE",
  );
  assert.equal(
    getReminderBucket({ ...input, effectiveDueDateKey: "2026-09-15" }),
    "DUE_TODAY",
  );
  assert.equal(
    getReminderBucket({ ...input, effectiveDueDateKey: "2026-10-15" }),
    "UPCOMING",
  );
  assert.equal(
    getReminderBucket({ ...input, effectiveDueDateKey: "2026-10-16" }),
    null,
  );
});
...
test("persiste una cadencia mensual y recupera un corte perdido una sola vez", () => {
  ...
  assert.deepEqual(
    first.map((period) => period.periodKey),
    ["2026-09"],
  );
  assert.deepEqual(
    getScheduledReminderPeriods({
