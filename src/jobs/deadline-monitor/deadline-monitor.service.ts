...
type MonitorParameters = {
  ...
  overdue_activity_enabled: boolean;
...
const readParameters = async (): Promise<MonitorParameters> => {
  ...
  return {
    ...
    overdue_activity_enabled: parseBoolean(
      values.get("overdue_activity_enabled"),
      DEADLINE_MONITOR_PARAMETER_DEFAULTS.overdue_activity_enabled,
    ),
...
const uniqueRecipients = (
  recipients: Array<Recipient | null | undefined>,
): Recipient[] => {
    ...
    if (!notification) {
      notification = await prisma.notification.create({
        data: {
          ...
          title: event.title,
          type:
            event.priority === NotificationPriority.CRITICAL
              ? "ERROR"
...
const notifyObservation = async (
  ...
  const recipients = uniqueRecipients([
    ...
    ...(context.parameters.notify_area_manager
      ? observation.areaAssignments.map(
...
const recordOverdueActivity = async (
  context: RunContext,
  observations: any[],
  actionPlans: any[],
): Promise<void> => {
  if (!context.parameters.overdue_activity_enabled) return;
  for (const observation of observations) {
    if (observation.status.isFinal) continue;
    try {
      await entityActivityService.create({
...
export const deadlineMonitorService = {
  async run(options: {
    ...
    try {
      ...
      const overdueObservations = observations.filter(
