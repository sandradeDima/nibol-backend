        ...
        text: joinTextBlocks(
          ...
          ...variables.reports.flatMap((report) => [
            ...
            ...report.observations.map(
              (item) => `${item.code ?? item.number}. ${item.title} · Área: ${item.area ?? "No especificada"} · Riesgo: ${item.risk} · Fecha compromiso: ${item.dueDate ?? "Por definir"}`,
            ),
          ]),
          `Total: ${variables.total}`,
          `Ingresar a la plataforma: ${variables.platformLink}`,
