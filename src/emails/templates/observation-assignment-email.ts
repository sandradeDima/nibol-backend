...
export const observationAssignmentEmailTemplate: EmailTemplateDefinition<"observationAssignment"> =
  {
    ...
    render: ({ brand, variables }) => {
      ...
      const context = variables.reports
        .map(
          (report) =>
            `Informe ${escapeHtml(report.reportNumber)} · ${escapeHtml(report.reportTitle)} · ${escapeHtml(report.areaNames.join(", "))}`,
        )
        .join("<br>");
      ...
      return {
        ...
        text: joinTextBlocks(
          ...
          `Tienes ${variables.total} observación${variables.total === 1 ? "" : "es"} nueva${variables.total === 1 ? "" : "s"} para gestionar en NIBOL.`,
          ...
          `Ver observaciones en NIBOL: ${variables.platformLink}`,
