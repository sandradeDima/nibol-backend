...
const bucketLabel = (bucket: "OVERDUE" | "DUE_TODAY" | "UPCOMING"): string =>
  bucket === "OVERDUE"
    ? "Vencidos"
    : bucket === "DUE_TODAY"
      ? "Vence hoy"
...
const deadlineBadgeStyle = (bucket: "OVERDUE" | "DUE_TODAY" | "UPCOMING") =>
  bucket === "OVERDUE"
...
export const deadlineReminderEmailTemplate: EmailTemplateDefinition<"deadlineReminder"> =
  {
    ...
    render: ({ brand, variables }) => {
      ...
      const sections = (["OVERDUE", "DUE_TODAY", "UPCOMING"] as const)
        .map((bucket) => {
      ...
      return {
        ...
        text: joinTextBlocks(
          ...
          `Al corte del ${variables.cutoffDate}, se identificaron planes de acción que requieren seguimiento.`,
          `Resumen: ${variables.overdue} vencidos · ${variables.dueToday} vencen hoy · ${variables.upcoming} próximos a vencer · ${variables.reprogrammed} reprogramados.`,
          ...(["OVERDUE", "DUE_TODAY", "UPCOMING"] as const).flatMap((bucket) => {
            const plans = variables.plans.filter((plan) => plan.bucket === bucket);
