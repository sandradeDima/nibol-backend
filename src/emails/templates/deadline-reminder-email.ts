...
const riskKey = (
  ...
  if (value.includes("medio") || value.includes("medium")) return "medium";
  if (value.includes("bajo") || value.includes("low")) return "low";
  return "other";
};

const renderRiskSummary = (
  plans: DeadlineReminderEmailVariables["plans"],
): string => {
  const counts = (["high", "medium", "low"] as const).map((key) => ({
    count: plans.filter((plan) => riskKey(plan) === key).length,
    key,
  }));
  const maximum = Math.max(1, ...counts.map(({ count }) => count));
  const labels = { high: "Alto", low: "Bajo", medium: "Medio" } as const;
  const colors = {
    ...
    render: ({ brand, variables }) => {
      ...
      return {
        ...
        text: joinTextBlocks(
          ...
          `Tienes ${variables.plans.length} plan${variables.plans.length === 1 ? "" : "es"} pendiente${variables.plans.length === 1 ? "" : "s"} en NIBOL.`,
          ...
          ...visiblePlans.map(
            (plan) =>
              `${plan.observation} · ${plan.plan} · ${plan.effectiveDueDate} · ${plan.risk} · ${bucketLabel(plan.bucket)}`,
          ),
          ...(omittedPlans > 0
