...
import { buildModuleNames, type ModuleNames } from "./casing.js";
import {
  renderBackendConstantsTemplate,
  renderBackendControllerTemplate,
  renderBackendPermissionsTemplate,
  renderBackendRoutesTemplate,
  renderBackendSchemaTemplate,
  renderBackendServiceTemplate,
  renderBackendTypesTemplate,
  renderBackendValidatorsTemplate,
  renderFrontendAppCreatePageTemplate,
  renderFrontendAppEditPageTemplate,
  renderFrontendAppListPageTemplate,
  renderFrontendAppViewPageTemplate,
  renderFrontendConstantsTemplate,
  renderFrontendCreatePageTemplate,
  renderFrontendEditPageTemplate,
  renderFrontendFormTemplate,
  renderFrontendHookTemplate,
  renderFrontendListPageTemplate,
  renderFrontendTableTemplate,
  renderFrontendTypesTemplate,
  renderFrontendViewPageTemplate,
} from "./templates.js";

type GenerateModuleScaffoldOptions = {
  backendRoot: string;
  moduleName: string;
};

type GeneratedModuleDescriptor = {
  label: string;
...
const toDisplayLabel = (routeSegment: string): string => {
  return routeSegment
    .split("-")
...
const collectGeneratedModules = async (
  backendModulesRoot: string,
): Promise<GeneratedModuleDescriptor[]> => {
...
const renderBackendRegistry = (
  ...
  const routerNames = modules.map((module) => {
    return `${module.routeSegment.replace(/-([a-z])/g, (_, letter: string) => letter.toUpperCase())}Router`;
  });
...
const rebuildGeneratedRegistries = async (
  backendRoot: string,
  frontendRoot: string,
): Promise<void> => {
  ...
  await writeFile(
    path.join(backendModulesRoot, "generated-module-registry.ts"),
    renderBackendRegistry(modules),
  );

  await writeFile(
    path.join(frontendRoot, "src", "modules", "generated-module-registry.ts"),
    renderFrontendRegistry(modules),
  );
};

const createBackendFiles = async (
  backendRoot: string,
  names: ModuleNames,
): Promise<void> => {
  ...
  await Promise.all([
    writeFile(
      path.join(moduleDirectory, `${names.fileStem}.constants.ts`),
      renderBackendConstantsTemplate(names),
    ),
    writeFile(
      path.join(moduleDirectory, `${names.fileStem}.permissions.ts`),
      renderBackendPermissionsTemplate(names),
    ),
    writeFile(
      path.join(moduleDirectory, `${names.fileStem}.schema.ts`),
      renderBackendSchemaTemplate(names),
    ),
    writeFile(
      path.join(moduleDirectory, `${names.fileStem}.validators.ts`),
      renderBackendValidatorsTemplate(names),
    ),
    writeFile(
      path.join(moduleDirectory, `${names.fileStem}.types.ts`),
      renderBackendTypesTemplate(names),
    ),
    writeFile(
      path.join(moduleDirectory, `${names.fileStem}.service.ts`),
      renderBackendServiceTemplate(names),
    ),
    writeFile(
      path.join(moduleDirectory, `${names.fileStem}.controller.ts`),
      renderBackendControllerTemplate(names),
    ),
    writeFile(
      path.join(moduleDirectory, `${names.fileStem}.routes.ts`),
      renderBackendRoutesTemplate(names),
    ),
  ]);
};

const createFrontendFiles = async (
  frontendRoot: string,
  names: ModuleNames,
): Promise<void> => {
  const moduleDirectory = path.join(
  ...
  await Promise.all([
    ...
    writeFile(
      path.join(componentsDirectory, `${names.entityLabelSingular}Form.tsx`),
      renderFrontendFormTemplate(names),
    ),
    writeFile(
    ...
    writeFile(
      path.join(appDetailDirectory, "page.tsx"),
      renderFrontendAppViewPageTemplate(names),
    ),
    writeFile(
      path.join(appEditDirectory, "page.tsx"),
      renderFrontendAppEditPageTemplate(names),
    ),
  ]);
};

export const generateModuleScaffold = async ({
  backendRoot,
  moduleName,
