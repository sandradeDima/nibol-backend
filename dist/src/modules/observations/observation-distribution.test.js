...
import { buildObservationAssignmentGroups, buildObservationSendDedupeKey, buildObservationSendOperationId, canSendObservation, formatRiskLevelMaxPeriods, } from "./observations.service.js";
const user = (id, email) => ({
    email,
    id,
    name: id,
});
const record = (id, number, areaAssignments) => ({
    areaAssignments,
...
const assignment = (areaId, processOwner, areaResponsible = processOwner) => ({
    actionPlans: [],
    area: { id: areaId, name: areaId },
    areaResponsible,
    processOwner,
});
const oneRecipientRecords = (count) => Array.from({ length: count }, (_, index) => record(`observation-${index + 1}`, index + 1, [
    assignment("Finanzas", user("owner", "OWNER@EXAMPLE.COM")),
]));
test("Case A: five observations for one recipient produce one email group", () => {
    const [group] = buildObservationAssignmentGroups(oneRecipientRecords(5), "Alto: 90 días");
...
test("Case B: two recipients only receive their relevant observations", () => {
    ...
    const groups = buildObservationAssignmentGroups(Array.from({ length: 5 }, (_, index) => record(`observation-${index + 1}`, index + 1, [
        assignment("Finanzas", owner),
        ...(index >= 3 ? [assignment("Operaciones", second)] : []),
    ])), "Alto: 90 días");
    assert.deepEqual(groups.map((group) => [group.email, group.observationIds.length]), [
        ["owner@example.com", 5],
...
test("Case C: role duplication for one person produces one group", () => {
    const owner = user("owner", "owner@example.com");
    const [group] = buildObservationAssignmentGroups([record("observation-1", 1, [assignment("Finanzas", owner, owner)])], "Alto: 90 días");
...
test("Case E: the send operation and delivery key are deterministic", () => {
    ...
    assert.equal(buildObservationSendDedupeKey(first, "owner", "EMAIL"), buildObservationSendDedupeKey(retry, "owner", "EMAIL"));
});
...
test("risk periods use active values in severity order", () => {
    assert.equal(formatRiskLevelMaxPeriods([
...
//# sourceMappingURL=observation-distribution.test.js.map