...
import { EDITABLE_PROGRESS_STATUSES, FILE_LEVEL_EVIDENCE_REVIEW_CONTEXTS, isOfficialProgressStatusTransitionAllowed, officialProgressByStatus, } from "../progress/progress.constants.js";
...
test("el conteo de tareas solo considera tareas de evaluaciones relacionadas", () => {
    assert.deepEqual(countWorkflowTasksForObservation(["evaluation-1", "evaluation-2"], new Map([
...
test("la revisión por archivo queda limitada a documentos de observación o cierre", () => {
    ...
    assert.equal(FILE_LEVEL_EVIDENCE_REVIEW_CONTEXTS.has("PROGRESS_EVALUATION"), false);
});
test("la revisión de un avance solo permite progresar el estado oficial", () => {
    assert.equal(isOfficialProgressStatusTransitionAllowed("STARTED", "WITH_PROGRESS"), true);
    assert.equal(isOfficialProgressStatusTransitionAllowed("WITH_PROGRESS", "STARTED"), false);
    assert.equal(isOfficialProgressStatusTransitionAllowed("WITH_PROGRESS", "WITH_PROGRESS"), true);
});
test("rechazar o devolver una tarea exige comentario", async () => {
    ...
    if (!extensionAdapter?.validateTaskAction ||
    ...
    await assert.rejects(validateExtensionTaskAction({ ...input, action: "REJECT" }), /comentario/);
    await assert.doesNotReject(() => validateClosureTaskAction({
        ...input,
...
test("el cierre no exige documento pero conserva el avance obligatorio", async () => {
    ...
    await assert.doesNotReject(() => adapter.validateStart({
        actorUserId: "user-1",
        db: db,
        entityId: "evaluation-1",
    }));
    reportedProgressPercent = 99;
    await assert.rejects(adapter.validateStart({
        actorUserId: "user-1",
        db: db,
        entityId: "evaluation-1",
    }), /100%/);
