...
import { createObservationSchema, listObservationsQuerySchema, observationIdParamSchema, sendObservationsSchema, updateObservationSchema, } from "./observations.validators.js";
...
const observationId = (request) => observationIdParamSchema.parse({ id: queryValue(request.params.id) }).id;
export const observationsController = {
    async sendObservations(request, response) {
        const result = await observationsService.sendObservations(sendObservationsSchema.parse(request.body).ids, requireAccess(request));
    ...
    async send(request, response) {
        const result = await observationsService.sendObservation(observationId(request), requireAccess(request));
        const actor = getRequestLogActorContext(request);
        await Promise.all([
            activityLogService.logUserAction({
                ...actor,
    ...
    async close(request, response) {
        ...
        const result = await observationsService.closeObservation(observationId(request), access);
        const actor = getRequestLogActorContext(request);
        await Promise.all([
            activityLogService.logUserAction({
                ...actor,
            ...
            entityActivityService.recordEntityChange({
                ...
                actorUserId: actor.userId,
                description: "Auditoría aprobó el cierre después de validar todos los planes de acción.",
                entityId: result.current.id,
                entityType: "OBSERVATION",
                newData: result.current,
                observationId: result.current.id,
    ...
    async create(request, response) {
        const access = requireAccess(request);
        const observation = await observationsService.createObservation(createObservationSchema.parse(request.body), access);
        const actor = getRequestLogActorContext(request);
        await Promise.all([
            activityLogService.logUserAction({
                ...actor,
    ...
    async getActionItems(request, response) {
        const access = requireAccess(request);
        const context = await observationsService.getObservationForActionItems(observationId(request), access);
        sendSuccess(response, await observationCompletenessService.getForObservation(context, access));
    },
    async getById(request, response) {
        sendSuccess(response, await observationsService.getObservationById(observationId(request), requireAccess(request)));
    },
    async list(request, response) {
        const query = listObservationsQuerySchema.parse({
            actionPlanResponsibleUserId: queryValue(request.query["filter.actionPlanResponsibleUserId"]),
            areaId: queryValue(request.query["filter.areaId"]),
            areaResponsibleUserId: queryValue(request.query["filter.areaResponsibleUserId"]),
    ...
    async options(request, response) {
        sendSuccess(response, await observationsService.getObservationFormOptions(requireAccess(request)));
    },
    async remove(request, response) {
        const actor = getRequestLogActorContext(request);
        const previous = await observationsService.deleteObservation(observationId(request), requireAccess(request));
        await Promise.all([
            activityLogService.logUserAction({
                ...actor,
    ...
    async update(request, response) {
        const result = await observationsService.updateObservation(observationId(request), updateObservationSchema.parse(request.body), requireAccess(request));
        const actor = getRequestLogActorContext(request);
        const activityType = result.previous.riskLevel.id !== result.current.riskLevel.id
