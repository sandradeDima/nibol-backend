...
import { OBSERVATION_OVERDUE_STATUS_KEY, OBSERVATION_TERMINAL_STATUS_KEYS, } from "./observations.constants.js";
...
const terminalStatusKeys = new Set(OBSERVATION_TERMINAL_STATUS_KEYS);
...
const hasGlobalObservationAccess = (access) => {
    ...
    if (access.permissions.includes(OBSERVATIONS_PERMISSIONS.create) ||
...
const buildEffectiveStatus = (status, dueDate) => {
...
const mapObservationListItem = (observation) => {
...
const mapObservationDetail = (observation) => {
...
const sanitizeAdditionalAreaIds = (areaId, additionalAreaIds) => {
    return Array.from(new Set(additionalAreaIds ?? [])).filter((value) => value !== areaId);
};
const buildAssignmentCreateInput = (areaId, responsibleUserId) => {
    return {
        area: {
            connect: {
                id: areaId,
...
const buildVisibilityCondition = (access) => {
...
const buildWhereClause = (query, access) => {
...
const assertCodeAvailable = async (code, observationId) => {
...
const assertReferencesExist = async (input) => {
    ...
    const [riskLevelCount, statusCount, areaCount, responsibleUserCount, processCatalogCount, typeCatalogCount, sourceCatalogCount, categoryCatalogCount,] = await Promise.all([
...
const findObservationDetailById = async (observationId, access) => {
    const visibilityCondition = buildVisibilityCondition(access);
...
const findObservationForMutation = async (observationId) => {
    const observation = await prisma.observation.findFirst({
...
export const observationsService = {
    async createObservation(input, access) {
        ...
        const additionalAreaIds = sanitizeAdditionalAreaIds(input.areaId, input.additionalAreaIds);
        ...
        const createData = {
            ...
            ...(additionalAreaIds.length > 0
                ? {
                    areaAssignments: {
                        create: additionalAreaIds.map((areaId) => buildAssignmentCreateInput(areaId, input.responsibleUserId)),
                    },
                }
                : {}),
            ...(input.responsibleUserId
    ...
    async getObservationById(observationId, access) {
    ...
    async updateObservation(observationId, input) {
        ...
        const nextValues = {
            additionalAreaIds: input.additionalAreaIds ??
                existingObservation.areaAssignments.map((assignment) => assignment.areaId),
            areaId: input.areaId ?? existingObservation.areaId,
            auditRecommendation: input.auditRecommendation ?? existingObservation.auditRecommendation,
            ...
            observationType: input.observationType === undefined
        ...
        const additionalAreaIds = sanitizeAdditionalAreaIds(nextValues.areaId, nextValues.additionalAreaIds);
        ...
        const shouldSyncAreaAssignments = input.additionalAreaIds !== undefined ||
            ...
            ...(shouldSyncAreaAssignments
                ? {
                    areaAssignments: {
                        ...
                        ...(additionalAreaIds.length > 0
                            ? {
                                create: additionalAreaIds.map((areaId) => buildAssignmentCreateInput(areaId, nextValues.responsibleUserId)),
                            }
                            : {}),
                    },
                }
