...
const hasGlobalAccess = (access) => access.isAdmin ||
    access.roles.some((role) => ["auditoria", "sistema", "sistemas"].includes(role
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .toLowerCase()));
export const buildObservationAccessWhere = (access) => {
...
export const observationsService = {
    ...
    async updateObservation(id, input, access) {
        ...
        const deadline = shouldRecalculateDeadline
            ? observationDeadlineService.calculate(auditReport.reportDate, riskLevel.key)
            : null;
