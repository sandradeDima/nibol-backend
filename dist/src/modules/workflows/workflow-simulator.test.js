...
test("simula una devolución controlada sin reejecutar la misma decisión en bucle", () => {
    ...
    assert.equal(correctionValidation.isValid, true, JSON.stringify(correctionValidation.errors));
    assert.equal(result.success, true);
    assert.equal(result.finalResult, "APPROVED");
    assert.equal(result.errors.length, 0);
    assert.ok(result.warnings.some((warning) => warning.message.includes("retorno controlado")));
    assert.ok(result.summary.visitedNodes < 10);
});
test("simula las rutas A-E del orquestador de observaciones", () => {
    const node = (nodeKey, type, configurationJson, positionX) => ({
        configurationJson,
        description: null,
        id: nodeKey,
    ...
    const base = (name, nodeType) => ({
        description: null,
        name,
        nodeType,
        schemaVersion: 1,
    });
    const condition = (nodeKey, field, operator, value) => node(nodeKey, "CONDITION", {
        ...base(nodeKey, "CONDITION"),
        defaultRouteLabel: "NO",
        logicalOperator: "AND",
        rules: [{ field, operator, resultLabel: null, value }],
    }, 0);
    const route = (id, sourceNodeId, targetNodeId, transitionType, conditionField, conditionValue, priority = 0) => ({
                ...
                conditions: [
                    {
                        ...
                        operator: typeof conditionValue === "number" ? "GREATER_THAN" : "EQUALS",
                        resultLabel: null,
                        sequence: 0,
    ...
    const stage = (nodeKey) => node(nodeKey, "STAGE", {
        ...base(nodeKey, "STAGE"),
    ...
    const graph = workflowDesignerSaveSchema.parse({
        nodes: [
            ...
            node("deadline-extension", "SUBFLOW", {
            ...
            node("extension-rejected", "NOTIFICATION", {
        ...
        transitions: [
            ...
            route("area-yes", "area", "create-area-plan", "CONDITION", "areaPlanRequired", true),
            ...
            route("extension-yes", "extension", "deadline-extension", "CONDITION", "requestedExtensionDays", 0),
            route("extension-no", "extension", "audit", "DEFAULT", undefined, undefined, 1),
            route("extension-result", "deadline-extension", "extension-result", "DEFAULT"),
            route("extension-rejected", "extension-result", "extension-rejected", "CONDITION", "previousDecision", "REJECTED"),
            route("extension-approved", "extension-result", "audit", "DEFAULT", undefined, undefined, 1),
            route("rejected-audit", "extension-rejected", "audit", "DEFAULT"),
            route("audit-approved", "audit", "all-plans", "APPROVE"),
            route("audit-correction", "audit", "correction", "REQUEST_CORRECTION", undefined, undefined, 1),
            route("correction-audit", "correction", "audit", "RETURN"),
            route("all-yes", "all-plans", "closure", "CONDITION", "allPlansValidated", true),
            route("all-no", "all-plans", "blocked", "DEFAULT", undefined, undefined, 1),
    ...
    for (const scenario of cases) {
        ...
        assert.equal(result.route.at(-1)?.nodeKey, scenario.expected, scenario.name);
        assert.equal(result.finalResult, scenario.expected === "finish" ? "APPROVED" : "RETURNED");
    }
});
