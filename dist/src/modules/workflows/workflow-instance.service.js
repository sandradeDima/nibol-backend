...
const isTerminal = (status) => ["COMPLETED", "REJECTED", "CANCELLED"].includes(status);
const canSeeInstance = async (db, instanceId, access) => {
    if (access.permissions.includes("workflows.view_instances") ||
...
const getInstanceForDetail = async (db, instanceId) => db.workflowInstance.findUnique({
    include: {
        currentNode: {
...
const parseArrayJson = (value) => Array.isArray(value) ? value : [];
...
export const workflowInstanceService = {
    async startInstance(input, access, options) {
        ...
        const createdId = await (await import("../../utils/prisma.js")).prisma.$transaction(async (db) => {
            ...
            if (definitions.length === 0) {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.NO_ACTIVE_VERSION, "No existe un workflow publicado y activo para este tipo de proceso.");
            }
            if (definitions.length > 1) {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.RUNTIME_CONFIGURATION, "Hay varios workflows activos para este tipo de proceso; seleccione un workflow explícitamente.");
            }
            const selected = definitions[0];
            if (!selected || selected.status !== "PUBLISHED" || selected.archivedAt) {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.RUNTIME_CONFIGURATION, "El workflow seleccionado está archivado o no está publicado.");
            }
            if (selected.processType !== input.processType) {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.RUNTIME_CONFIGURATION, "El workflow seleccionado no corresponde al tipo de proceso solicitado.");
            }
            const version = selected.activeVersion;
            if (!version || selected.activeVersionId !== version.id) {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.NO_ACTIVE_VERSION, "El workflow no tiene una versión activa.");
            }
            if (version.status !== "PUBLISHED") {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.VERSION_NOT_PUBLISHED, "La versión activa del workflow no está publicada.");
            }
    ...
    async getHistory(instanceId, access) {
        const detail = await this.getInstance(instanceId, access);
        return detail.timeline;
    },
    async getInstance(instanceId, access) {
    ...
    async cancelInstance(instanceId, access) {
        ...
        return prisma.$transaction(async (db) => {
            ...
            if (isTerminal(instance.status) || instance.status === "FAILED") {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.INSTANCE_NOT_ACTIONABLE, "La instancia ya no puede cancelarse en su estado actual.");
            }
            const now = new Date();
    ...
    async retryInstance(instanceId, access) {
        ...
        await prisma.$transaction(async (db) => {
            ...
            if (instance.status !== "FAILED") {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.INSTANCE_NOT_ACTIONABLE, "Solo se pueden reintentar instancias fallidas.");
            }
            const graph = await loadPinnedWorkflowGraph(db, instance.workflowVersionId);
            const node = instance.currentNodeId
                ? graph.nodes.find((candidate) => candidate.id === instance.currentNodeId)
                : null;
            if (!node || node.type === "STAGE" || node.type === "APPROVAL") {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.INSTANCE_NOT_ACTIONABLE, "El reintento solo está disponible para fallos de nodos automáticos.");
            }
            const now = new Date();
            await db.workflowInstance.update({
                data: {
    ...
    async getInstanceWithinTransaction(db, instanceId) {
