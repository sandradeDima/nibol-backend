...
const approvalWithSla = (overrides = {}) => {
    const baseSla = {
...
test("calcula el calendario de timers de una tarea con SLA", () => {
    ...
    assert.deepEqual(schedule.timers.map((timer) => [
        timer.timerType,
...
test("una tarea sin SLA no crea timers y los días hábiles conservan la política UTC", () => {
    ...
    assert.deepEqual(calculateWorkflowTimerSchedule({
        configuration: noSla,
        entrySequence: 1,
        sourceNodeId: "node-audit",
    ...
    assert.equal(calculateWorkflowTimerSchedule({
        configuration: businessDay,
        entrySequence: 1,
        sourceNodeId: "node-audit",
        startedAt: new Date("2026-08-07T10:00:00.000Z"),
    }).dueAt?.toISOString(), "2026-08-10T10:00:00.000Z");
});
//# sourceMappingURL=workflow-timer.test.js.map