import { z } from "zod";
import { WORKFLOW_ASSIGNMENT_STRATEGY_VALUES, WORKFLOW_CONDITION_FIELD_ALIASES, WORKFLOW_CONDITION_FIELD_INPUT_VALUES, WORKFLOW_CONDITION_LOGIC_VALUES, WORKFLOW_CONDITION_OPERATOR_VALUES, WORKFLOW_DEFINITION_STATUS_VALUES, WORKFLOW_NODE_TYPE_VALUES, WORKFLOW_PROCESS_TYPE_VALUES, WORKFLOW_TRANSITION_TYPE_VALUES, WORKFLOW_VERSION_STATUS_VALUES, } from "./workflows.constants.js";
...
const workflowConditionValueSchema = z
    ...
    z.number().finite(),
    z.boolean(),
    z.array(z.union([z.string().trim().max(1_000), z.number().finite(), z.boolean()])),
    z.null(),
])
    .optional();
const workflowConditionFieldSchema = z
    .string()
    .trim()
    .refine((value) => WORKFLOW_CONDITION_FIELD_INPUT_VALUES.includes(value), "El campo de la regla no está soportado.")
    .transform((value) => {
    const alias = WORKFLOW_CONDITION_FIELD_ALIASES[value];
    return (alias ?? value);
});
const workflowConditionRuleSchema = z
    ...
    .strict();
const validateConditionRuleCompatibility = (rule, context) => {
    const value = rule.value;
    const noValueOperator = ["IS_EMPTY", "IS_NOT_EMPTY", "IS_OVERDUE"].includes(rule.operator);
    if (noValueOperator &&
        value !== undefined &&
        value !== null &&
        value !== "") {
...
const compatibleConditionRuleSchema = workflowConditionRuleSchema.superRefine(validateConditionRuleCompatibility);
...
const rejectionNodeConfigurationSchema = configurationBaseSchema
    ...
    .strict()
    .refine((value) => value.behavior !== "RETURN_TO_STAGE" ||
...
const endNodeConfigurationSchema = configurationBaseSchema
    ...
    .strict();
export const workflowNodeConfigurationSchema = z.discriminatedUnion("nodeType", [
...
export const workflowNodeSaveSchema = z
    ...
    if (value.assignmentStrategy &&
        ...
        context.addIssue({
            code: "custom",
            message: "La estrategia de asignación debe coincidir con la configuración.",
...
const workflowConditionGroupSaveSchema = z
    ...
    conditions: z
        .array(compatibleConditionRuleSchema.extend({
...
//# sourceMappingURL=workflows.validators.js.map