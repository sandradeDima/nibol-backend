...
const transition = (id, sourceNodeId, targetNodeId, transitionType, priority) => ({
    id,
    label: null,
    priority,
    sourceNodeId,
...
const parse = (graph) => workflowDesignerSaveSchema.parse(graph);
...
test("requiere la ruta fallback cuando se declaran reglas de condición", () => {
    ...
    const result = validateWorkflowGraph(graph, options);
    assert.ok(result.errors.some((issue) => issue.code === "CONDITION_FALLBACK_REQUIRED"));
});
test("valida un subflujo publicado y bloquea la autorreferencia", () => {
    ...
    const selfReferencing = validateWorkflowGraph(parse({
        nodes: [
            start,
            {
                ...subflow,
    ...
    assert.ok(selfReferencing.errors.some((issue) => issue.code === "SUBFLOW_SELF_REFERENCE"));
});
