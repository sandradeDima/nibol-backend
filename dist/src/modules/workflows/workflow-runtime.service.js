...
import { resolveRuntimeAssignment, } from "./workflow-assignment-resolver.js";
...
import { loadPinnedWorkflowGraph, getRuntimeNode, } from "./workflow-runtime-graph.js";
import { WORKFLOW_RUNTIME_ERROR_CODES, WorkflowRuntimeError, } from "./workflow-runtime-errors.js";
import { selectRuntimeTransition, summarizeConditionEvaluations, } from "./workflow-transition-resolver.js";
...
import { buildWorkflowInstanceUrl, createTaskAssignedNotification, createWorkflowCompletionNotification, createWorkflowNotificationIntent, getWorkflowTaskAssignmentRecipientsForTask, resolveWorkflowNotificationRecipients, } from "./workflow-notification.service.js";
import { createWorkflowTaskTimers } from "./workflow-timer.service.js";
const asJson = (value) => toLogJsonValue(value) ?? {};
const getAssignmentSnapshot = (resolution) => ({
...
const ensureHumanTask = async ({ actor, context, db, instanceId, node, now, }) => {
    if (!node || (node.type !== "STAGE" && node.type !== "APPROVAL")) {
        throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.RUNTIME_CONFIGURATION, "El runtime intentó crear una tarea para un nodo no accionable.");
    }
    ...
    if (configuration.nodeType !== "STAGE" &&
        ...
        throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.RUNTIME_CONFIGURATION, "La configuración de la tarea no corresponde a una etapa humana.");
    }
    ...
    if (!resolution.assignedAreaId &&
        !resolution.assignedRoleId &&
        !resolution.assignedUserId) {
        throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.ASSIGNMENT_UNRESOLVED, `No se pudo resolver un responsable para la etapa ${node.name}.`, 409, { nodeKey: node.nodeKey, strategy: resolution.strategy });
    }
    ...
    const assignmentRecipients = await getWorkflowTaskAssignmentRecipientsForTask(db, task.id);
...
const markRuntimeFailure = async ({ actor, context, db, error, instanceId, nodeId, now, }) => {
    await db.workflowInstance.update({
        data: {
            lastExecutionAt: now,
...
const mapEndResultToStatus = (finalResult) => {
...
const completeAtEnd = async ({ actor, context, db, instanceId, instance, node, now, }) => {
    if (!node || node.configurationJson.nodeType !== "END") {
        throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.RUNTIME_CONFIGURATION, "El runtime intentó completar una instancia fuera de un nodo final.");
    }
    ...
    try {
        ...
        if (error instanceof WorkflowRuntimeError)
            throw error;
        throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.INTEGRATION_FAILED, error instanceof AppError
            ? error.message
            : "No se pudo actualizar el registro relacionado al completar el workflow.", 409);
    }
    await db.workflowInstance.update({
        data: {
            completedAt: now,
...
const isTerminalRuntimeStatus = (status) => ["COMPLETED", "REJECTED", "CANCELLED"].includes(status);
const executeAutomaticNodesInternal = async ({ actor = null, db, instanceId, now = new Date(), }) => {
    ...
    if (!instance) {
        throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.INSTANCE_NOT_ACTIONABLE, "La instancia de workflow no existe.", 404);
    }
    ...
    if (!start) {
        const error = new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.START_NODE_MISSING, "La versión publicada no contiene un nodo Inicio válido.");
        const context = restoreWorkflowRuntimeContext(instance.processType, instance.contextJson);
        await markRuntimeFailure({ actor, context, db, error, instanceId, now });
        return { status: "FAILED" };
    }
    let context = restoreWorkflowRuntimeContext(instance.processType, instance.contextJson);
    let currentNodeId = instance.currentNodeId ?? start.id;
    let currentNode = getRuntimeNode(graph, currentNodeId);
    if (!currentNode) {
        const error = new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.RUNTIME_CONFIGURATION, "La instancia apunta a un nodo que no existe en su versión fijada.");
        await markRuntimeFailure({ actor, context, db, error, instanceId, now });
        return { status: "FAILED" };
    }
    await db.workflowInstance.update({
    ...
    for (let step = 0; step < WORKFLOW_GRAPH_LIMITS.maxSimulationSteps; step += 1) {
        if (!currentNode)
            break;
        ...
        if (currentNode.configurationJson.nodeType === "START" &&
            currentNode.configurationJson.processType !== instance.processType) {
            const error = new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.RUNTIME_CONFIGURATION, "El nodo Inicio no corresponde al tipo de proceso de la instancia.");
        ...
        if (!selection.selected) {
            const error = new WorkflowRuntimeError(selection.errorCode ??
                WORKFLOW_RUNTIME_ERROR_CODES.TRANSITION_NOT_FOUND, selection.errorMessage ?? "No se encontró una ruta de ejecución.");
            await markRuntimeFailure({
                actor,
                context,
                db,
        ...
        if (!targetNode) {
            const error = new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.TRANSITION_NOT_FOUND, "La ruta publicada apunta a un nodo inexistente.");
            await markRuntimeFailure({
                actor,
                context,
                db,
        ...
        await writeRuntimeTransitionLog({
            ...
            db,
            details: {
                conditionEvaluations: summarizeConditionEvaluations(selection.conditionEvaluations),
    ...
    const error = new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.RUNTIME_STEP_LIMIT, `La ejecución superó el máximo de ${WORKFLOW_GRAPH_LIMITS.maxSimulationSteps} pasos.`);
    await markRuntimeFailure({
        actor,
        context,
        db,
...
export const executeAutomaticNodes = async (input) => {
