...
import { createWorkflowDecisionNotification, deliverWorkflowNotificationsForInstance, } from "./workflow-notification.service.js";
...
import { workflowTaskActionPermissions, } from "./workflow-runtime.validators.js";
...
const isTaskActor = async (db, task, userId) => {
...
const getTaskRecord = async (db, taskId) => db.workflowTask.findUnique({
    include: {
...
const getNodeConfiguration = (configurationJson) => {
    const parsed = workflowNodeConfigurationSchema.safeParse(configurationJson);
    if (!parsed.success) {
        throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.RUNTIME_CONFIGURATION, "La configuración publicada de la tarea no es válida.");
    }
    return parsed.data;
};
const getTaskDueState = (dueAt, timers) => {
    const now = new Date();
    const reminderAt = timers.find((timer) => timer.timerType === "REMINDER" && timer.status !== "CANCELLED")?.executeAt;
    return getWorkflowSlaState({
        dueAt,
        dueSoonAt: reminderAt ??
...
const assertActionRequirements = ({ action, comment, configuration, context, evidenceReferences, }) => {
    if (configuration.nodeType !== "STAGE" &&
        ...
        throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.ACTION_NOT_ALLOWED, "Esta tarea no corresponde a una etapa humana.");
    }
    if (!configuration.allowedActions.includes(action)) {
        throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.ACTION_NOT_ALLOWED, `La acción ${action} no está disponible para esta tarea.`);
    }
    const commentRequired = configuration.nodeType === "STAGE"
        ? configuration.requiredComment
        : configuration.commentRequired;
    const evidenceRequired = configuration.nodeType === "STAGE"
        ? configuration.requiredEvidence
        : configuration.evidenceRequired;
    if (commentRequired && !comment?.trim()) {
        throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.ACTION_NOT_ALLOWED, "Debe ingresar un comentario para completar esta acción.", 400);
    }
    if (evidenceRequired &&
        context.hasEvidence !== true &&
        !(context.evidenceCount && context.evidenceCount > 0) &&
        !(evidenceReferences && evidenceReferences.length > 0)) {
        throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.ACTION_NOT_ALLOWED, "Esta acción requiere evidencia disponible en el contexto o referencias controladas.", 400);
    }
};
const mapTask = (task, access, canAct) => {
    if (!task)
        return null;
...
const buildTaskWhere = (userId, query) => {
    const now = new Date();
    const defaultDueSoonAt = new Date(now.getTime() + WORKFLOW_DEFAULT_DUE_SOON_HOURS * 3_600_000);
    const activeReminder = {
        status: { not: "CANCELLED" },
        timerType: "REMINDER",
    };
    const dueStateWhere = query.dueState === "OVERDUE"
        ? { dueAt: { lt: now } }
...
export const workflowTaskService = {
    async listMyPending(query, access) {
        ...
        return {
            data: tasks.map((task) => {
                ...
                return {
                    ...
                    assignedRole: task.assignedRole,
                    assignedUser: task.assignedUser,
                    assignmentStrategy: typeof assignmentRecord?.strategy === "string"
                    ...
                    priority: null,
                    status: task.status,
                    allowedActions: configuration.nodeType === "STAGE" ||
                        configuration.nodeType === "APPROVAL"
                        ? configuration.allowedActions
                        : [],
                };
    ...
    async actOnTask(taskId, action, input, access) {
        ...
        const result = await prisma.$transaction(async (db) => {
            ...
            if (!taskIsActionable(task.status)) {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.TASK_ALREADY_COMPLETED, "La tarea ya fue atendida y no admite otra decisión.", 409);
            }
            if (!["ACTIVE", "WAITING"].includes(task.instance.status)) {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.INSTANCE_NOT_ACTIONABLE, "La instancia ya no está disponible para decisiones.");
            }
            if (!(await isTaskActor(db, task, access.userId))) {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.TASK_UNAUTHORIZED, "No está autorizado para actuar sobre esta tarea.", 403);
            }
            const graph = await loadPinnedWorkflowGraph(db, task.instance.workflowVersionId);
            const node = graph.nodes.find((candidate) => candidate.id === task.nodeId);
            if (!node) {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.RUNTIME_CONFIGURATION, "La tarea apunta a un nodo que no existe en la versión fijada.");
            }
            ...
            if (!selection.selected) {
                throw new WorkflowRuntimeError(selection.errorCode ??
                    WORKFLOW_RUNTIME_ERROR_CODES.TRANSITION_NOT_FOUND, selection.errorMessage ?? "No se encontró una ruta para la decisión.");
            }
            const targetNode = graph.nodes.find((candidate) => candidate.id === selection.selected?.targetNodeId);
            if (!targetNode) {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.TRANSITION_NOT_FOUND, "La decisión apunta a un nodo inexistente.");
            }
            ...
            if (claimed.count !== 1) {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.TASK_ALREADY_COMPLETED, "La tarea ya fue atendida por otra solicitud.", 409);
            }
    ...
    async reassignTask(taskId, input, access) {
        ...
        return prisma.$transaction(async (db) => {
            ...
            if (!task)
                throw new AppError("No se encontró la tarea solicitada.", 404);
            if (!taskIsActionable(task.status)) {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.TASK_ALREADY_COMPLETED, "Solo se pueden reasignar tareas activas.");
            }
            const configuration = getNodeConfiguration(task.node.configurationJson);
            if (configuration.nodeType !== "STAGE" &&
                configuration.nodeType !== "APPROVAL") {
                throw new WorkflowRuntimeError(WORKFLOW_RUNTIME_ERROR_CODES.ACTION_NOT_ALLOWED, "Solo se pueden reasignar tareas humanas.");
            }
            ...
            if (input.assignedRoleId) {
                ...
                if (!role)
                    throw new AppError("El rol destino no está activo.", 400);
            }
            if (input.assignedAreaId) {
                ...
                if (!area)
                    throw new AppError("El área destino no está activa.", 400);
            }
            const now = new Date();
