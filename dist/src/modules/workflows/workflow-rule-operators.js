import { getWorkflowRuleField, normalizeWorkflowRuleValue, } from "./workflow-rule-fields.js";
const isEmpty = (value) => {
    return (value === null ||
        value === undefined ||
        value === "" ||
...
const comparePrimitive = (actual, expected, operator) => {
    switch (operator) {
        case "EQUALS":
...
const normalizeExpectedValue = (field, operator, value) => {
...
export const validateWorkflowRule = (rule) => {
    const field = getWorkflowRuleField(rule.field);
    if (!field) {
        return "El campo de la regla no está soportado.";
    }
    if (!field.allowedOperators.includes(rule.operator)) {
        return `El operador ${rule.operator} no es compatible con ${field.label}.`;
    }
    const operator = rule.operator;
    const noValueOperator = ["IS_EMPTY", "IS_NOT_EMPTY", "IS_OVERDUE"].includes(operator);
    if (noValueOperator) {
...
const evaluateDateOperator = (actualValue, expectedValue, operator, now) => {
...
export const evaluateWorkflowRule = (rule, context, options, sequence = 0) => {
    const field = getWorkflowRuleField(rule.field);
    const operator = rule.operator;
    ...
    const matched = operator === "IS_EMPTY"
        ? isEmpty(actualValue)
        : operator === "IS_NOT_EMPTY"
            ? !isEmpty(actualValue)
            : field.dataType === "date"
                ? evaluateDateOperator(actualValue, expectedValue, operator, options.now)
...
//# sourceMappingURL=workflow-rule-operators.js.map