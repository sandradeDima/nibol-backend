...
import { getActionPlanDeadlineStatus, getEffectiveActionPlanDueDate, getOfficialActionPlanProgress, getObservationStatusGroup, getRiskGroupLabel, isApprovedDeadlineExtension, isObservationDueSoon, isObservationOverdue, reportDeadlineStatusOptions, } from "./reporting-definitions.js";
...
test("mantiene el estado oficial separado del avance reportado", () => {
    assert.deepEqual(["NOT_STARTED", "STARTED", "WITH_PROGRESS", "CONCLUDED"].map(getOfficialActionPlanProgress), [
...
test("aplica la fecha efectiva aprobada y conserva la fecha para hoy como vigente", () => {
    ...
    assert.equal(getEffectiveActionPlanDueDate(plan).toISOString(), "2026-09-09T00:00:00.000Z");
    assert.equal(getActionPlanDeadlineStatus(plan, new Date("2026-09-08T12:00:00.000Z"), "UTC"), "VIGENTE");
    assert.equal(getActionPlanDeadlineStatus(plan, new Date("2026-09-09T23:59:59.000Z"), "UTC"), "VIGENTE");
    assert.equal(getActionPlanDeadlineStatus(plan, new Date("2026-09-10T12:00:00.000Z"), "UTC"), "VENCIDO");
    assert.equal(isApprovedDeadlineExtension({ status: "MANAGER_APPROVED" }), true);
    ...
    assert.equal(getEffectiveActionPlanDueDate({
        currentDueDate: new Date("2026-09-01T00:00:00.000Z"),
        deadlineExtensionRequests: [
            {
    ...
    }).toISOString(), "2026-09-01T00:00:00.000Z");
    assert.equal(getActionPlanDeadlineStatus({
        ...plan,
        deadlineExtensionRequests: [
...
test("solo pendientes filtra por estado de observación y no por estado del plan", () => {
    const serialized = JSON.stringify(buildActionPlanWhere({ activeOnly: true }, scopedExecutor, now, "UTC"));
...
test("los filtros multiselección aceptan CSV y conservan el estado Todos vacío", () => {
    const parsed = reportFiltersSchema.parse({
        deadlineStatuses: "VIGENTE,VENCIDO",
        observationStatusIds: "11111111-1111-4111-8111-111111111111,22222222-2222-4222-8222-222222222222",
    });
