...
import { buildObservationScopeWhere, getActionPlanDeadlineStatus, getBusinessDateKey, getDateOnlyKey, getEffectiveActionPlanDueDate, getOfficialActionPlanProgress, reportDeadlineStatusOptions, isApprovedDeadlineExtension, OFFICIAL_ACTION_PLAN_STATUSES, getDaysBetween, getObservationStatusGroup, getRiskGroupLabel, hasGlobalBusinessAccess, isObservationClosed, isObservationDueSoon, isObservationOverdue, REPORT_DEFAULT_DUE_SOON_DAYS, } from "./reporting-definitions.js";
import { AppError } from "../../utils/app-error.js";
const reportsPrisma = prisma;
export const getReportFilterCapabilities = (access) => {
    const global = access.isAdmin ||
...
export const assertReportFilterAccess = (filters, access) => {
    ...
    if (requested.some(([isRequested, isAllowed]) => isRequested && !isAllowed) ||
        (filters.responsibleUserId &&
            !capabilities.executor &&
            filters.responsibleUserId !== access.userId)) {
        throw new AppError("Uno de los filtros no está disponible para su alcance.", 403);
    }
};
...
const observationDisplayCode = (observation) => `${observation.auditReport.reportNumber} / OBS-${String(observation.observationNumber).padStart(3, "0")}`;
...
const buildObservationWhere = (filters, access) => {
    ...
    if (filters.activeOnly)
        conditions.push({ status: { isFinal: false } });
    ...
    if (filters.observationStatusIds?.length)
        ...
        conditions.push({ statusId: filters.statusId });
    ...
    if (filters.hasPlan !== undefined) {
        conditions.push(filters.hasPlan
    ...
    if (filters.overdue) {
        conditions.push(buildObservationAttentionWhere("OVERDUE", filters.dueSoonDays));
    }
    if (filters.dueSoon) {
        conditions.push(buildObservationAttentionWhere("DUE_SOON", filters.dueSoonDays));
    }
    return { AND: conditions };
};
const getConfiguredDueSoonDays = async () => {
    try {
...
const getDateAtBusinessTimeZone = (value, timeZone) => {
...
const getNextDateKey = (value) => {
    const date = toDateAtUtc(value);
    date.setUTCDate(date.getUTCDate() + 1);
    return getDateOnlyKey(date);
};
const getCutoffAsOfDate = (cutoffDate, timeZone) => new Date(getDateAtBusinessTimeZone(cutoffDate, timeZone).getTime() +
    12 * 60 * 60 * 1000);
...
export const buildActionPlanWhere = (filters, access, now = new Date(), timeZone = "UTC") => {
    const cutoffDateKey = filters.cutoffDate ?? getBusinessDateKey(now, timeZone);
    const cutoffDate = toDateAtUtc(cutoffDateKey);
    const cutoffEnd = getDateAtBusinessTimeZone(getNextDateKey(cutoffDateKey), timeZone);
    const conditions = [
    ...
    else if (deadlineStatuses?.has("VIGENTE") &&
...
const mapActionPlanRow = (record, asOf, timeZone) => {
...
const loadActionPlanRows = async (filters, access) => {
    ...
    return {
        cutoffDate,
        dueSoonDays,
        rows: records.map((record) => mapActionPlanRow(record, getCutoffAsOfDate(cutoffDate, timeZone), timeZone)),
        timeZone,
    };
};
...
export const buildPlanDistributions = (rows) => ({
    area: distributionFromRows(rows, (row) => row.area.id, (row) => row.area.name, (row) => `/reportes?filter.areaId=${encodeURIComponent(row.area.id)}`),
    areaResponsible: distributionFromRows(rows, (row) => row.areaResponsible?.id ?? "unassigned", (row) => row.areaResponsible?.name ?? "Sin asignar", (row) => row.areaResponsible
        ? `/reportes?filter.areaResponsibleId=${encodeURIComponent(row.areaResponsible.id)}`
        : undefined),
    executor: distributionFromRows(rows, (row) => row.executor?.id ?? "unassigned", (row) => row.executor?.name ?? "Sin asignar", (row) => row.executor
        ? `/reportes?filter.executorId=${encodeURIComponent(row.executor.id)}`
        : undefined),
    processOwner: distributionFromRows(rows, (row) => row.processOwner?.id ?? "unassigned", (row) => row.processOwner?.name ?? "Sin asignar", (row) => row.processOwner
        ? `/reportes?filter.processOwnerId=${encodeURIComponent(row.processOwner.id)}`
        : undefined),
});
const isCriticalRisk = (row) => {
    const value = `${row.riskLevel.key} ${row.riskLevel.name}`.toUpperCase();
    return (value.includes("CRIT") || value.includes("ALTO") || value.includes("HIGH"));
};
const buildOperationalRows = (rows, cutoffDateKey, dueSoonDays) => {
    const cutoffDate = toDateAtUtc(cutoffDateKey);
    const criticalOrOverdue = new Map();
    rows.forEach((row) => {
        const observationOverdue = isObservationOverdue(new Date(row.observationDueDate), row.observation.status, cutoffDate);
        if (!isCriticalRisk(row) &&
            !observationOverdue &&
    ...
    const upcomingActionPlans = rows
        ...
        const diff = getDaysBetween(cutoffDate, toDateAtUtc(getDateOnlyKey(new Date(row.effectiveDueDate))));
    ...
    return {
        criticalOrOverdueObservations: [...criticalOrOverdue.values()].sort((left, right) => left.dueDate.localeCompare(right.dueDate)),
        upcomingActionPlans,
    };
};
const getActionPlanDashboard = async (filters, access) => {
    ...
    rows.forEach((row) => observations.set(row.observation.id, row.observation.status));
    const totalObservations = observations.size;
    const closedObservations = [...observations.values()].filter((status) => status.isFinal).length;
    ...
    const observationStatusDistribution = [...observations.values()]
        ...
        .values();
    const statusDistribution = [...observationStatusDistribution].sort((left, right) => right.value - left.value || left.label.localeCompare(right.label, "es"));
    const dueSoon = rows.filter((row) => {
        if (row.officialProgress.key === "CONCLUDED" ||
            row.deadlineStatus === "VENCIDO")
            return false;
        const diff = getDaysBetween(toDateAtUtc(loaded.cutoffDate), toDateAtUtc(getDateOnlyKey(new Date(row.effectiveDueDate))));
    ...
    const deadlineDistribution = reportDeadlineStatusOptions.map(({ key, label }) => ({
        key,
        label,
    ...
    const months = getMonthRange(filters.dateFrom, filters.dateTo, loaded.cutoffDate);
    const trend = months.map((month) => {
        const monthKey = getMonthKey(month);
        const value = monthRows.get(monthKey) ?? { closed: 0, created: 0 };
        return { ...value, label: getMonthLabel(month), monthKey };
    });
    const topResponsibleWorkload = distributionFromRows(rows.filter((row) => row.officialProgress.key !== "CONCLUDED" && row.areaResponsible), (row) => row.areaResponsible.id, (row) => row.areaResponsible.name).slice(0, 6);
    const topOverdueAreas = distributionFromRows(rows.filter((row) => row.deadlineStatus === "VENCIDO"), (row) => row.area.id, (row) => row.area.name).slice(0, 6);
    ...
    return {
        ...
        insights,
        operational: buildOperationalRows(rows, loaded.cutoffDate, loaded.dueSoonDays),
        rows,
        summary: {
...
const getActionPlanRows = async (query, access) => {
    ...
    return {
        data: records.map((record) => mapActionPlanRow(record, getCutoffAsOfDate(cutoffDate, timeZone), timeZone)),
        total,
    };
};
const toActionPlanFlatRow = (row) => ({
    ...
    "Estado según plazo": row.deadlineStatus === "VENCIDO" ? "Vencido" : "Vigente",
...
const getReportOptions = async (access) => {
    const [{ cutoffDate, rows }, riskLevels, observationStatuses] = await Promise.all([
    ...
    rows.forEach((row) => {
        const relationship = relationshipMap.get(row.area.id) ?? {
            ...
            responsibleExecutorIds: new Map(),
        };
        if (row.areaResponsible) {
            relationship.areaResponsibleIds.add(row.areaResponsible.id);
            const executorIds = relationship.responsibleExecutorIds.get(row.areaResponsible.id) ??
                new Set();
            if (row.executor)
                executorIds.add(row.executor.id);
            relationship.responsibleExecutorIds.set(row.areaResponsible.id, executorIds);
        }
        if (row.executor)
            relationship.executorIds.add(row.executor.id);
        if (row.processOwner)
            relationship.processOwnerIds.add(row.processOwner.id);
        relationshipMap.set(row.area.id, relationship);
    });
    const areaRelationships = [...relationshipMap.values()].map((relationship) => ({
        ...
        responsibleExecutorIds: [...relationship.responsibleExecutorIds].map(([areaResponsibleId, executorIds]) => ({
            areaResponsibleId,
    ...
    return {
        ...
        areaResponsibles: unique(rows.flatMap((row) => (row.areaResponsible ? [row.areaResponsible] : []))),
        auditReports,
...
const mapObservationRow = (observation, actionSummary, now = new Date()) => {
    const overdue = isObservationOverdue(observation.currentDueDate, observation.status, now);
    return {
...
const mapObservationRows = async (observations, access, now = new Date()) => {
    const actionSummaryById = await observationCompletenessService.getSummaries(observations.map(buildObservationActionInput), access);
    return observations.map((observation) => mapObservationRow(observation, actionSummaryById.get(observation.id), now));
};
const getObservationRows = async (query, access, options) => {
    const dueSoonDays = query.dueSoonDays ?? (await getConfiguredDueSoonDays());
...
const buildClosureMap = async (where) => {
...
const formatReportDateKey = (value) => new Intl.DateTimeFormat("es-BO", {
    day: "2-digit",
    month: "2-digit",
    timeZone: "UTC",
    year: "numeric",
}).format(toDateAtUtc(value));
const getMonthRange = (dateFrom, dateTo, cutoffDate) => {
    ...
    const months = [];
    for (const current = first; current <= last; current.setUTCMonth(current.getUTCMonth() + 1)) {
        months.push(new Date(current));
    }
    return months.slice(-12);
};
const buildFilterLabelMap = (filters, dashboard) => {
...
const getDashboard = async (filters, access) => {
    ...
    const [total, open, closed, overdue, dueSoon, inProcess, observations, closureMap,] = await Promise.all([
        reportsPrisma.observation.count({ where }),
    ...
    observations.forEach((observation) => {
        const observationOverdue = isObservationOverdue(observation.currentDueDate, observation.status, now);
        const observationDueSoon = isObservationDueSoon(observation.currentDueDate, observation.status, dueSoonDays, now);
        const statusGroup = getObservationStatusGroup(observation.status, observationOverdue);
        const riskLabel = getRiskGroupLabel(observation.riskLevel.key, observation.riskLevel.name);
        ...
        for (const assignment of observation.areaAssignments) {
            ...
            if (observation.status.key === "INICIADO" ||
                observation.status.key === "CON_AVANCE")
                currentArea.inProcess += 1;
            if (observationOverdue)
                currentArea.overdue += 1;
            if (observationDueSoon)
                currentArea.dueSoon += 1;
            areaMap.set(assignment.areaId, currentArea);
        }
    });
    const areaSummary = [...areaMap.values()].map(({ complianceCount: areaComplianceCount, resolutionCount, ...area }) => ({
        ...area,
        averageResolutionDays: resolutionCount > 0
            ? Math.round(area.averageResolutionDays / resolutionCount)
            : 0,
        compliancePercent: resolutionCount > 0
    ...
    const trend = months.map((month) => {
        const monthKey = getMonthKey(month);
        const created = observations.filter((observation) => getMonthKey(observation.createdAt) === monthKey).length;
        const closedCount = [...closureMap.entries()].filter(([observationId, closedAt]) => {
            return (getMonthKey(closedAt) === monthKey &&
    ...
    const riskDistribution = [...riskCounts.values()].sort((left, right) => right.value - left.value);
    const statusDistribution = [...statusCounts.values()].sort((left, right) => right.value - left.value);
    const predominantRisk = riskDistribution[0]
        ? {
            count: riskDistribution[0].value,
            key: riskDistribution[0].key,
    ...
    const averageResolutionDays = resolutionDurations.length > 0
        ? Math.round(resolutionDurations.reduce((sum, value) => sum + value, 0) /
    ...
    const compliancePercent = complianceDenominator > 0
        ...
        : 0;
    const insights = [];
    const overdueArea = [...areaSummary].sort((left, right) => right.overdue - left.overdue)[0];
    if (overdueArea && overdueArea.overdue > 0) {
        insights.push(`${overdueArea.area.name} concentra el mayor número de observaciones vencidas (${overdueArea.overdue}).`);
    }
    const slowestArea = [...areaSummary].sort((left, right) => right.averageResolutionDays - left.averageResolutionDays)[0];
    if (slowestArea && slowestArea.averageResolutionDays > 0) {
        insights.push(`${slowestArea.area.name} presenta el mayor tiempo promedio de resolución: ${slowestArea.averageResolutionDays} días.`);
    }
    const highRiskDueSoon = observations.filter((observation) => {
        const risk = getRiskGroupLabel(observation.riskLevel.key, observation.riskLevel.name);
        return (["Crítico", "Alto"].includes(risk) &&
            isObservationDueSoon(observation.currentDueDate, observation.status, dueSoonDays, now));
    }).length;
    if (highRiskDueSoon > 0) {
        insights.push(`${highRiskDueSoon} observación${highRiskDueSoon === 1 ? "" : "es"} de riesgo Alto o Crítico vence${highRiskDueSoon === 1 ? "" : "n"} en los próximos ${dueSoonDays} días.`);
    }
    if (insights.length === 0 && total > 0) {
        insights.push("No se identificaron concentraciones críticas con los filtros seleccionados.");
    }
    return {
        areaSummary,
        charts: {
...
const toObservationFlatRow = (row) => ({
    Área: row.area.name,
...
const getReportRows = async (query, access, dashboard) => {
    ...
    switch (query.type) {
        ...
        case "RESPONSIBLES": {
            ...
            observations.forEach((observation) => {
                const name = observation.areaAssignments[0]?.areaResponsible?.name ??
                    "Sin asignar";
                ...
                if (!isObservationClosed(observation.status))
                    group.open += 1;
                if (isObservationOverdue(observation.currentDueDate, observation.status))
                    group.overdue += 1;
...
const getActivityLabel = (activityType, action) => {
    ...
    return (labels[normalized] ?? labels[action.toUpperCase()] ?? "Actividad registrada");
};
const getActivityResult = (activityType) => {
    const normalized = activityType.toUpperCase();
...
const getBusinessProcessLabel = (value) => {
    const labels = {
...
const getBusinessWorkflowResult = (value) => {
    if (!value)
        return "Registrado";
...
const buildAuditObservationWhere = (query, access) => {
    const where = buildObservationScopeWhere(access);
    const conditions = [where];
    if (query.observationId)
        conditions.push({ id: query.observationId });
    if (query.areaId)
        conditions.push({ areaAssignments: { some: { areaId: query.areaId } } });
    if (query.riskLevelId)
        conditions.push({ riskLevelId: query.riskLevelId });
    if (query.status)
        conditions.push({ status: { key: query.status } });
    return { AND: conditions };
};
const getAuditHistory = async (query, access) => {
    ...
    const normalizedRows = activities.map((activity) => ({
    ...
    const filteredRows = normalizedRows.filter((row) => {
        ...
        if (query.template === "APPROVALS") {
            return ["Aprobado", "Rechazado", "Corrección solicitada"].includes(row.result);
        }
        return true;
    });
    if (query.template === "ACTIVITY_AREA") {
        filteredRows.sort((left, right) => left.area.localeCompare(right.area, "es") ||
            left.date.localeCompare(right.date));
    }
    if (query.template === "ACTIVITY_USER") {
        filteredRows.sort((left, right) => left.actor.localeCompare(right.actor, "es") ||
            left.date.localeCompare(right.date));
    }
...
const getAuditStructuredRows = async (query, access) => {
    const observationWhere = buildAuditObservationWhere(query, access);
    const dateFilter = dateRange(query.dateFrom, query.dateTo);
    const common = {
        ...(dateFilter ? { createdAt: dateFilter } : {}),
    ...
    if (query.template === "EXTENSIONS" || query.template === "DEADLINES") {
        ...
        return {
            columns: query.template === "DEADLINES"
                ? [
                    "Observación",
                    "Área",
                    "Responsable",
            ...
            rows: requests.map((request) => {
                const observation = request.observation ?? request.actionPlan?.observation;
                const extensionDays = Math.max(0, Math.round((request.proposedDueDate.getTime() -
                ...
                return query.template === "DEADLINES"
                    ? {
                        ...
                        "Días vencidos": effectiveDueDate.getTime() < Date.now()
                            ? Math.round((Date.now() - effectiveDueDate.getTime()) /
                                (1000 * 60 * 60 * 24))
                            : 0,
                        Estado: request.status,
    ...
    if (query.template === "INCUMPLIMIENTOS") {
        ...
        return {
            ...
            rows: observations.map((observation) => ({
                ...
                "Días vencidos": Math.max(0, Math.round((Date.now() - observation.currentDueDate.getTime()) /
                    (1000 * 60 * 60 * 24))),
                "Fecha límite": observation.currentDueDate.toISOString(),
                Estado: "Vencida",
                Observación: `${observationDisplayCode(observation)} · ${observation.title}`,
                Responsable: observation.areaAssignments[0]?.areaResponsible.name ?? "Sin asignar",
                Riesgo: observation.riskLevel.name,
            })),
    ...
    if (query.template === "WORKFLOW_HISTORY") {
        const shouldScopeEntities = !hasGlobalBusinessAccess(access) ||
            Boolean(query.observationId ||
                query.areaId ||
                query.riskLevelId ||
                query.status);
        let entityIds;
        if (shouldScopeEntities) {
            ...
            entityIds = scopedObservations.flatMap((observation) => [
                observation.id,
                ...observation.deadlineExtensionRequests.map((item) => item.id),
                ...observation.progressEvaluations.map((item) => item.id),
                ...observation.remediationPlans.map((item) => item.id),
            ]);
        }
        ...
        return {
            ...
            rows: transitions.map((transition) => ({
                ...
                Etapa: transition.targetNode?.name ??
...
export const reportsService = {
    ...
    async getPreview(query, access) {
        const dashboard = query.type === "ACTION_PLANS"
            ? await getActionPlanDashboard(query, access)
            : await getDashboard(query, access);
        const report = query.type === "ACTION_PLANS"
    ...
    async getAuditReport(query, access) {
        if (query.template === "HISTORY" ||
