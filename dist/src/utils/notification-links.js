...
export const buildPendingWorkUrl = () => "/planes-accion?filter.status=NOT_STARTED,STARTED,WITH_PROGRESS";
export const buildFrontendUrl = (path) => {
    const base = `${env.FRONTEND_URL.replace(/\/+$/, "")}/`;
    return new URL(path, base).toString();
};
export const resolveNotificationTarget = (input) => {
    ...
    const workflowTaskId = input.workflowTaskId ??
        (entityType.includes("workflow_task") ? entityId : null);
    if (workflowTaskId)
        return `/aprobaciones/flujos/${pathId(workflowTaskId)}`;
    const extensionId = input.extensionId ?? (entityType.includes("extension") ? entityId : null);
    ...
    const actionPlanId = input.actionPlanId ??
        (entityType.includes("action_plan") || entityType.includes("remediation")
            ? entityId
            : null);
    if (actionPlanId)
        return `/planes-accion/${pathId(actionPlanId)}`;
    ...
    if (observationId) {
        return buildObservationUrl(observationId, {
            ...(eventType.includes("assignment") ? { tab: "summary" } : {}),
        });
    }
    if (eventType.includes("observation"))
        return "/observaciones";
    if (entityType.includes("deadline_reminder") ||
...
//# sourceMappingURL=notification-links.js.map