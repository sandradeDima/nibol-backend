...
const assertUserCanLoseActiveAccess = async (userId) => {
...
const assertEmailAvailable = async (email, excludedUserId) => {
...
const removePreviousAvatarIfLocal = async (avatarUrl) => {
    if (!avatarUrl) {
        return;
    }
...
export const usersService = {
    async bulkDeleteUsers(ids, context) {
        ...
        await Promise.all(existingUsers.map((user) => assertUserCanLoseActiveAccess(user.id)));
        const deletedUsers = await prisma.$transaction(async (transaction) => {
            const snapshots = (await Promise.all(existingUsers.map((user) => getUserAuditSnapshot(transaction, user.id)))).filter((snapshot) => snapshot !== null);
    ...
    async changePassword(userId, input, context) {
    ...
    async updateUser(userId, input, context) {
        const existingUser = await prisma.user.findFirst({
            select: {
                email: true,
                id: true,
