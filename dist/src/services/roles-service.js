...
const buildDeletedRoleName = (roleName, roleId) => {
...
const assertRoleNameAvailable = async (roleName, excludedRoleId) => {
...
const getPermissionRecords = async (permissionNames) => {
...
const getRoleDetailsRecord = async (roleId) => {
...
const getRoleAuditSnapshot = async (db, roleId) => {
...
export const rolesService = {
    async createRole(input, context) {
    ...
    async deleteRole(roleId, context) {
    ...
    async updateRole(roleId, input, context) {
        const existingRole = await prisma.role.findFirst({
            select: {
                id: true,
            },
        ...
        await adminSafeguardService.assertRolePermissionUpdateAllowed(roleId, input.permissionNames);
        const permissions = await getPermissionRecords(input.permissionNames);
        await prisma.$transaction(async (transaction) => {
            const previousSnapshot = await getRoleAuditSnapshot(transaction, roleId);
            await transaction.role.update({
            ...
            if (previousSnapshot && nextSnapshot) {
                ...
                if (!areStringArraysEqual(previousSnapshot.permissionNames, nextSnapshot.permissionNames)) {
                    await activityLogService.logUserAction({
                        ...context,
                        action: "Permission changed",
                        entityId: roleId,
