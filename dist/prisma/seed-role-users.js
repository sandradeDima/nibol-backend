...
const rolePassword = process.env.SEED_ROLE_PASSWORD ??
    process.env.SEED_DEMO_PASSWORD ??
    (isProduction ? "" : "nibol-demo-local-change-me");
const adminPassword = process.env.SEED_ADMIN_PASSWORD ?? (isProduction ? "" : "Mipassword!");
if (rolePassword.length < 8 ||
    adminPassword.length < 8 ||
    !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(process.env.SEED_ADMIN_EMAIL ?? "admin@gmail.com")) {
    throw new Error("Set valid SEED_ROLE_PASSWORD, SEED_ADMIN_PASSWORD, and SEED_ADMIN_EMAIL values before seeding.");
}
...
const getPassword = (roleCode) => roleCode === ADMIN_ROLE_CODE ? adminPassword : rolePassword;
const seed = async () => {
    const summary = await prisma.$transaction(async (tx) => {
        ...
        for (const role of ROLE_DEFINITIONS) {
            ...
            if (!roleId)
                throw new Error(`Role ${role.code} was not seeded.`);
            ...
            await tx.rolePermission.createMany({
                data: ROLE_PERMISSION_NAMES[role.code].map((name) => {
                    ...
                    return {
                        id: uuidv5(`role-permission:${roleId}:${permissionId}`, SEED_NAMESPACE),
                        permissionId,
                        roleId,
                    };
                }),
        ...
        for (const roleUser of roleUsers) {
            ...
            const user = await tx.user.upsert({
                create: {
                    ...
                    emailVerified: true,
                    id: uuidv5(`role-user:${roleUser.email.toLowerCase()}`, SEED_NAMESPACE),
                    isActive: true,
            ...
            if (account) {
                ...
                await tx.account.create({
                    data: {
                        accountId: user.id,
                        id: uuidv5(`role-account:${roleUser.email.toLowerCase()}`, SEED_NAMESPACE),
                        password,
...
//# sourceMappingURL=seed-role-users.js.map